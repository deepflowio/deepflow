...
const (
	_U160_KEY_SIZE = 160 / 8 // 必须为4字节的倍数
)

type u160LRUNode struct {
	key   [_U160_KEY_SIZE]byte
...
func (n *u160LRUNode) equal(hash uint32, key []byte) bool {
	return n.hash == hash && bytes.Compare(n.key[:], key) == 0
}

var blankU160LRUNodeForInit u160LRUNode

type u160LRUNodeBlock []u160LRUNode

var u160LRUNodeBlockPool = sync.Pool{New: func() interface{} {
	return u160LRUNodeBlock(make([]u160LRUNode, _BLOCK_SIZE))
}}

// 注意：不是线程安全的
type U160LRU struct {
	ringBuffer       []u160LRUNodeBlock // 存储Map节点，以矩阵环的方式组织，提升内存申请释放效率
...
func (m *U160LRU) Size() int {
	return m.size
}

func (m *U160LRU) incIndex(index int32) int32 {
	index++
...
func (m *U160LRU) decIndex(index int32) int32 {
	if index <= 0 {
...
func (m *U160LRU) getNode(index int32) *u160LRUNode {
	return &m.ringBuffer[index>>_BLOCK_SIZE_BITS][index&_BLOCK_SIZE_MASK]
}

func (m *U160LRU) pushNodeToHashList(node *u160LRUNode, nodeIndex int32, hash int32) {
...
func (m *U160LRU) pushNodeToTimeList(node *u160LRUNode, nodeIndex int32) {
...
func (m *U160LRU) removeNodeFromHashList(node *u160LRUNode, newNext, newPrev int32) {
...
func (m *U160LRU) removeNodeFromTimeList(node *u160LRUNode, newNext, newPrev int32) {
...
func (m *U160LRU) removeNode(node *u160LRUNode, nodeIndex int32) {
	// 从哈希链表、时间链表中删除
	...
	if nodeIndex != m.bufferStartIndex {
		...
		*node = *firstNode
		...
		// 将firstNode初始化
		*firstNode = blankU160LRUNodeForInit
	} else {
		*node = blankU160LRUNodeForInit
	}

	// 释放头部节点
	if m.bufferStartIndex&_BLOCK_SIZE_MASK == _BLOCK_SIZE_MASK {
		u160LRUNodeBlockPool.Put(m.ringBuffer[m.bufferStartIndex>>_BLOCK_SIZE_BITS])
...
func (m *U160LRU) updateNode(node *u160LRUNode, nodeIndex int32, value interface{}) {
...
func (m *U160LRU) newNode(hash uint32, key []byte, value interface{}) {
	// buffer空间检查
	...
	if m.ringBuffer[row] == nil {
		m.ringBuffer[row] = u160LRUNodeBlockPool.Get().(u160LRUNodeBlock)
	}
...
func (m *U160LRU) Add(key []byte, value interface{}) {
	if len(key) != _U160_KEY_SIZE {
		panic(fmt.Sprintf("传入key的长度不等于 %d 字节", _U160_KEY_SIZE))
	}
...
func (m *U160LRU) Remove(key []byte) {
	if len(key) != _U160_KEY_SIZE {
		panic(fmt.Sprintf("传入key的长度不等于 %d 字节", _U160_KEY_SIZE))
	}
...
func (m *U160LRU) Get(key []byte, peek bool) (interface{}, bool) {
	if len(key) != _U160_KEY_SIZE {
		panic(fmt.Sprintf("传入key的长度不等于 %d 字节", _U160_KEY_SIZE))
	}
...
func (m *U160LRU) Clear() {
	for i := range m.ringBuffer {
		if m.ringBuffer[i] != nil {
			u160LRUNodeBlockPool.Put(m.ringBuffer[i])
			m.ringBuffer[i] = nil
		}
	}
...
func (m *U160LRU) genHash(key []byte) uint32 {
	hash := uint32(0)
	for i := 0; i < _U160_KEY_SIZE; i += 4 {
		hash ^= BigEndian.Uint32(key[i:])
	}
	return hash
}

func (m *U160LRU) compressHash(hash uint32) int32 {
	return int32((((((hash>>m.hashSlotBits)^hash)>>m.hashSlotBits)^hash)>>m.hashSlotBits)^hash) & (m.hashSlots - 1)
}

func NewU160LRU(hashSlots, capacity int) *U160LRU {
	hashSlots, hashSlotBits := minPowerOfTwo(hashSlots)

	m := &U160LRU{
		ringBuffer:   make([]u160LRUNodeBlock, (capacity+_BLOCK_SIZE)/_BLOCK_SIZE*_BLOCK_SIZE+1),
