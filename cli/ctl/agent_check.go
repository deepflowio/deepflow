...
func agentRegiterCommand() []*cobra.Command {
	...
	tapTypesCmd := &cobra.Command{
		Use:   "capture_network_type",
		Short: "get capture_network_type from deepflow-server",
		Run: func(cmd *cobra.Command, args []string) {
	...
	commands := []*cobra.Command{platformDataCmd, ipGroupsCmd, flowAclsCmd,
		configCmd, tapTypesCmd, segmentsCmd, containersCmd, skipInterfaceCmd,
		gpidAgentResponseCmd, gpidGlobalTableCmd, gpidAgentRequestCmd,
