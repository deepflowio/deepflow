...
func agentRegiterCommand() []*cobra.Command {
	...
	grpcBufferSizeCmd := &cobra.Command{
		...
		Short: "get grpc buffer size from deepflow-server",
		Run: func(cmd *cobra.Command, args []string) {
...
func AgentCheckRegisterCommand() *cobra.Command {
	...
	trisolarisCmd.PersistentFlags().Uint64VarP(&agentParamData.GRPCBufferSize, "size", "", 1, "agent grpc buffer size, Unit: MB")
...
func agentInitCmd(cmd *cobra.Command, cmds []AgentCmdExecute) {
	...
	if agentParamData.GRPCBufferSize == 1 {
