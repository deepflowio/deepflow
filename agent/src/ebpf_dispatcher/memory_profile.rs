...
impl Processor {
    ...
    pub fn process(&mut self, data: vec::Drain<Box<Data>>) {
        for d in data {
            ...
            if profile.wide_count as i64 <= 0 {
                // frees
                if let Some(AllocInfo { stack_id, size }) = self.allocated_addrs.pop(&AddrKey {
                    ...
                    self.initial_memory = self
                        .initial_memory
                        .saturating_sub((profile.wide_count as i64).abs() as u64);
                }
            } else {
                // allocs
...
impl MemoryContext {
    pub unsafe fn update(
        ...
        if let Err(e) = self.queue.send(Box::new(if data.count as i64 <= 0 {
            // frees
            Data {
                profile: metric::Profile {
...
impl Interior {
    ...
    fn report(&mut self, config: &EbpfProfileMemory, compress: bool, timestamp: Duration) {
        ...
        for (_, mut profile) in self.processor.allocs.drain() {
            ...
            if compress {
                match zstd::bulk::compress(&profile.data, 0) {
                    Ok(compressed_data) => {
        ...
        self.processor
            ...
            .retain(|StackKey { pid, .. }, profile| {
                ...
                if self.processor.initial_memory > 0
                    && *pid == agent_pid
                    && agent_initial_memory.is_none()
                {
                    agent_initial_memory = Some(metric::Profile {
                        ...
                        u_stack_id: 0,
                        cpu: 0,
                        count: self.processor.initial_memory as u32,
                        wide_count: self.processor.initial_memory,
                ...
                let mut p = metric::Profile {
                    timestamp: doc_timestamp,
                    event_type: metric::ProfileEventType::EbpfMemInUse.into(),
                    count: profile.wide_count as u32,
                    data: vec![],
                    ..profile.clone()
                };
                if compress {
                    match zstd::bulk::compress(&profile.data, 0) {
                        ...
                        Err(e) => {
                            ...
                            p.data_compressed = false;
                            p.data = profile.data.clone();
                        }
                    }
                } else {
                    p.data_compressed = false;
        ...
        while self.running.load(Ordering::Relaxed) {
                ...
                .recv_all(&mut batch, Some(Self::QUEUE_RECV_TIMEOUT))
            {
                Ok(_) => {
                    ...
                    ts_nanos = self.cache.last().map(|it| it.profile.timestamp);
                }
            ...
            // use max timestamp in self.cache if the result of `recv_all` is not timeout,
