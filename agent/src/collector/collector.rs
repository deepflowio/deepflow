...
fn get_edge_tagger(
    ...
    let (src_ip, dst_ip) = match running_mode {
...
impl Collector {
    ...
    pub fn start(&self) {
        ...
        let running_mode = config.load().running_mode;
        let thread = thread::Builder::new()
            .name("collector".to_owned())
            .spawn(move || {
                let mut stash = Stash::new(ctx, sender, counter, running_mode);
                let mut batch = Vec::with_capacity(QUEUE_BATCH_SIZE);
