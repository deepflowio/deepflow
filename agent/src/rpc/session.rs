...
macro_rules! sync_grpc_call_unary {
    ($self:ident, $func:ident, $request:ident, $endpoint:ident) => {{
        ...
        if let Ok(message) = &response {
            $self.set_message_size(message.get_ref().encoded_len());
        };
...
pub struct Session {
    ...
    cache_counter: Arc<GrpcCacheCounter>,
}
...
impl Session {
    pub fn new(
        ...
        let cache_counter = Arc::new(GrpcCacheCounter::default());
        ...
        stats_collector.register_countable(
            &stats::NoTagModule("grpc_cache"),
            Countable::Ref(Arc::downgrade(&cache_counter) as Weak<dyn RefCountable>),
        );
    ...
    pub fn set_message_size(&self, size: usize) {
        let _ = self.cache_counter.message_size.fetch_update(
            Ordering::Acquire,
            Ordering::Relaxed,
            |x| {
                if size as u64 > x {
    ...
    pub async fn grpc_get_plugin(
        ...
        while let Some(message) = msg.message().await? {
            ...
            self.set_message_size(message.encoded_len());
...
#[derive(Default)]
pub struct GrpcCacheCounter {
    pub buffer_size: AtomicU64,
    pub message_size: AtomicU64,
}

impl RefCountable for GrpcCacheCounter {
    fn get_counters(&self) -> Vec<Counter> {
        let buffer_size = self.buffer_size.load(Ordering::Relaxed) as u64;
        let message_size = self.message_size.swap(0, Ordering::Relaxed) as u64;

        vec![
            (
                "buffer_size",
                CounterType::Gauged,
                CounterValue::Unsigned(buffer_size),
            ),
            (
                "message_size",
                CounterType::Gauged,
                CounterValue::Unsigned(message_size),
            ),
        ]
    }
}
