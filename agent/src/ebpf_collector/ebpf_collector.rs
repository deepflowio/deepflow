...
use crate::flow_generator::{
    protocol_logs::AppProtoLogsImpl, protocol_logs::L7ProtocolInfoInterface, AppProtoLogsBaseInfo,
    AppProtoLogsData, AppTable, Error as LogError, Result as LogResult,
};
...
impl SessionAggr {
    ...
    fn send(&self, log: AppProtoLogsData) {
        ...
        let _ = self.output.send(SendItem::L7FlowLog(Box::new(log.into())));
    ...
    */
    fn slot_handle(&mut self, log: AppProtoLogsData, slot_index: usize, key: u64) {
        if log.is_request() {
            if log.special_info.need_merge() {
    ...
    fn on_non_merge_request_log(&mut self, mut log: AppProtoLogsData, slot_index: usize, key: u64) {
        ...
        if item.is_response() {
            ...
            if let Err(LogError::L7LogCanNotMerge(item)) = log.merge(item) {
                self.send(item);
            }
            self.cache_count -= 1;
            self.send(log);
    ...
    fn on_merge_request_log(&mut self, log: AppProtoLogsData, slot_index: usize, key: u64) {
        ...
        let _ = item.merge(log);

        let (request, response) = item.special_info.is_req_resp_end();
        if request && response {
            self.cache_count -= 1;
            self.send(item);
        } else {
    ...
    fn on_non_merge_response_log(&mut self, log: AppProtoLogsData, slot_index: usize, key: u64) {
        ...
        if item.is_response() {
            ...
            // if can not merge, send req and resp directly.
            // generally use for ebpf disorder.
            if let Err(LogError::L7LogCanNotMerge(log)) = item.merge(log) {
                self.send(log);
            }
            self.cache_count -= 1;
            self.send(item);
...
impl FlowItem {
    ...
    fn handle(
        ...
        if let Ok(logs) = self.parse(packet, local_epc, app_table, log_parser_config) {
            let mut result = vec![];
            for log in logs.into_iter() {
                if let Some(header) = log.app_proto_head() {
