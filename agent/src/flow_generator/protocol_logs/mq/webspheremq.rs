...
use enterprise_utils::l7::mq::webspheremq::WebSphereMqParser;
...
impl WebSphereMqInfo {
    pub fn merge(&mut self, other: &mut Self) {
        ...
        swap_if!(self, segment_type, is_empty, other);
    ...
    fn set_is_on_blacklist(&mut self, config: &LogParserConfig) {
        if let Some(t) = config.l7_log_blacklist_trie.get(&L7Protocol::WebSphereMQ) {
            self.is_on_blacklist = t.request_type.is_on_blacklist(&self.segment_type)
        }
    }

    pub fn merge_custom_info(&mut self, custom: CustomInfo) {
        if custom.msg_type != LogMessageType::default() {
            self.msg_type = custom.msg_type;
        }
        // req rewrite
        ...
        if !custom.req.req_type.is_empty() {
            self.segment_type = custom.req.req_type;
        }
...
const SEGMETNT_TYPE_MQINPUT: u8 = 0x86;
impl L7ProtocolParserInterface for WebSphereMqLog {
    ...
    fn parse_payload(&mut self, payload: &[u8], param: &ParseParam) -> Result<L7ParseResult> {
        if !self.parser.parse_payload(payload) {
            return Err(Error::L7ProtocolUnknown);
        }

        if self.parser.segment_type != SEGMETNT_TYPE_MQINPUT && param.wasm_vm.borrow().is_none() {
            log::info!("websphere parse_payload false");
            return Err(Error::L7ProtocolUnknown);
        }

        let mut info = WebSphereMqInfo::default();

        info.segment_type = format!("0x{:02x}", self.parser.segment_type);
        info.msg_type = LogMessageType::Request;
        if let Some(code) = &self.parser.msg_prc_cd {
            info.msg_type = LogMessageType::Response;
            if code.ends_with(RESPONSE_CODE_OK_SUFFIX) {
                info.status = L7ResponseStatus::Ok;
            } else {
                info.response_exception = code.to_string();
        ...
        if self.parser.segment_type != SEGMETNT_TYPE_MQINPUT
            && param.wasm_vm.borrow().is_some()
            && !self.wasm_hook(param, payload, &mut info)
        {
            log::info!("websphere parse_payload custom false");
            return Err(Error::L7ProtocolUnknown);
        }
        log::info!("websphere parse_payload true");
