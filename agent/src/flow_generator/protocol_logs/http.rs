...
const V1_HEADER_END: &'static [u8] = b"\r\n\r\n";
...
mod tests {
    ...
    fn test_openai_normal_usage_pcap() {
        ...
        assert!(
            matches!(session.usage_status, openai_api::UsageStatus::Available),
    ...
    fn test_openai_stream_pcap() {
        ...
        for info in &results {
            if let Some(session) = &info.openai_session {
                ...
                assert!(
                    matches!(session.kind, openai_api::OpenAiKind::ChatCompletions),
    ...
    fn test_openai_metrics_in_send_log() {
        use openai_api::{OpenAiKind, OpenAiSession, OpenAiUsage, UsageStatus};
