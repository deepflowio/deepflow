...
impl L7ProtocolParserInterface for HttpLog {
    ...
    fn parse_payload(&mut self, payload: &[u8], param: &ParseParam) -> Result<L7ParseResult> {
        ...
        #[cfg(feature = "enterprise")]
        let (policy, policy_indicies) = match self.proto {
            L7Protocol::Http1 => {
                let policy = config
                    .l7_log_dynamic
                    .extra_field_policies
                    .get(&L7ProtocolEnum::L7Protocol(self.proto));
                let policy_indicies = match config
                    .l7_log_dynamic
                    .extra_segment_map
                    .get(&L7ProtocolEnum::L7Protocol(self.proto))
                {
                    Some(segment_map) => segment_map.find(port),
                    None => None,
                };
                (policy, policy_indicies)
            }
            L7Protocol::Http2 | L7Protocol::Grpc => {
                let policy = config
                    .l7_log_dynamic
...
impl HttpLog {
    ...
    pub fn check_http2_go_uprobe(
        ...
        #[cfg(feature = "enterprise")] policy_indicies: Option<&Vec<usize>>,
    ) -> Result<usize> {
        ...
        #[cfg(feature = "enterprise")]
        {
            self.on_header_extra(
                ...
                tags,
                policy,
                policy_indicies.clone(),
                key,
                val,
            );
            if key == b":path" {
                self.on_http_url(info, direction, tags, policy, policy_indicies);
            }
        }
    ...
    pub fn parse_http_v1(
        ...
        if direction == PacketDirection::ServerToClient {
            ...
            #[cfg(feature = "enterprise")]
            self.on_http_url(info, direction, tags, policy, policy_indicies.clone());
        }
    ...
    fn on_http_url(
        ...
        policy_indicies: Option<&Vec<usize>>,
    ) {
        let (Some(policies), Some(policy_indices)) = (policy, policy_indicies) else {
            return;
        };
            ...
            for (field_type, fields) in field_policy {
                ...
                if payload_entry_cache.is_none() {
                    match format_payload(field_type, payload) {
                        Some(payload_entry) => payload_entry_cache = Some(payload_entry),
                        None => continue,
                    }
                }
    ...
    fn test_extra_field_policy_in_payload() {
        ...
        let config = L7LogDynamicConfig::new(
            ...
            HashMap::from([(
                L7ProtocolEnum::L7Protocol(L7Protocol::Http1),
                vec![ExtraCustomFieldPolicy {
                    from_req_body: HashMap::from([(FieldType::PayloadJson, rewrite_body_fields)]),
                    ..Default::default()
                }],
            )]),
        );
