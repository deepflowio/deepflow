...
use public::{
    ...
    l7_protocol::{CustomProtocol, L7Protocol},
};
...
impl L7ProtocolParserInterface for CustomPolicyLog {
    fn check_payload(&mut self, payload: &[u8], param: &ParseParam) -> bool {
        ...
        if config.custom_protocol_config.is_empty() {
            return false;
        }
        for (p, e) in &config.custom_protocol_config {
            println!(
                "custom_protocol_config: {:?}, req: {:?}, resp: {:?}",
                p, e.request_characters, e.response_characters
            );
        }
        ...
        let (Some(custom_protocol_name), checked) =
            self.parser
    ...
    fn parse_payload(&mut self, payload: &[u8], param: &ParseParam) -> Result<L7ParseResult> {
        ...
        let Some(custom_field_policy) = config
            .l7_log_dynamic
            .extra_field_policies
            .get(&L7Protocol::Custom)
        else {
            return Err(Error::NoParseConfig);
        };
