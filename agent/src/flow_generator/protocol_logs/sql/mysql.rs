...
impl MysqlInfo {
    ...
    fn request_string(&mut self, config: Option<&LogParserConfig>, payload: &[u8]) {
        let context = mysql_string(payload);
...
impl MysqlLog {
    ...
    fn request(
        ...
        match info.command {
            ...
            COM_INIT_DB | COM_QUERY | COM_STMT_PREPARE => {
                info.request_string(config, &payload[COMMAND_OFFSET + COMMAND_LEN..]);
            }
            COM_PING => {}
    ...
    fn parse(
        ...
        match msg_type {
            LogMessageType::Request => self.request(config, &payload[offset..], info)?,
