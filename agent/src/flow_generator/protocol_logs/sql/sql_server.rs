...
impl SqlServerInfo {
    ...
    fn set_is_on_blacklist(&mut self, config: &LogParserConfig) {
        if let Some(t) = config.l7_log_blacklist_trie.get(&L7Protocol::SqlServer) {
            self.is_on_blacklist = t.request_resource.is_on_blacklist(&self.sql.as_str())
                || t.request_type.is_on_blacklist(&self.request_type.as_str());
        }
    }
}
    ...
    fn parse_payload(&mut self, payload: &[u8], param: &ParseParam) -> Result<L7ParseResult> {
        ...
        match param.direction {
            PacketDirection::ClientToServer => {
                ...
                info.sql = sql;
                if let Some(request_type) = tds.request_type {
                    info.request_type = request_type.to_string();
                }
                if let Some(endpoint) = tds.endpoint {
