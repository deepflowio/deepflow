    ...
    fn get_timeout(config: &LogParserConfig, app_proto: &MetaAppProto) -> Timestamp {
            ...
            .get(&app_proto.base_info.head.proto)
        {
            ...
            None => match app_proto.base_info.head.proto {
                L7Protocol::DNS => ApplicationTimeout::DNS_DEFAULT,
                L7Protocol::TLS => ApplicationTimeout::TLS_DEFAULT,
                _ => ApplicationTimeout::DEFAULT,
            },
        }
        .into()
    }
        ...
        let thread = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                while running.load(Ordering::Relaxed) {
                    ...
                    if config.l7_log_session_aggr_max_timeout != session_queue.max_timelines {
                        session_queue.max_timelines = config.l7_log_session_aggr_max_timeout;
                    }
