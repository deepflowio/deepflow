...
pub struct DebugInfo(&'static str);
...
impl From<&'static str> for DebugInfo {
...
struct ComponentState {
    ...
    last_transition_mono_ms: AtomicU64,
    last_debug: Mutex<DebugInfo>,
}
...
impl ComponentState {
    fn new(spec: ComponentSpec, generation: u64) -> Self {
        Self {
            ...
            last_transition_mono_ms: AtomicU64::new(u64::MAX),
            last_debug: Mutex::new(DebugInfo::default()),
        }
    }
    ...
    fn snapshot(&self, now_mono_ms: u64) -> ComponentSnapshot {
        ...
        let last_transition_mono_ms = self.last_transition_mono_ms.load(Ordering::Relaxed);
        ComponentSnapshot {
            ...
            last_transition_ago_ms: if last_transition_mono_ms == u64::MAX {
                None
            } else {
                Some(now_mono_ms.saturating_sub(last_transition_mono_ms))
            },
            last_debug: self.last_debug.lock().as_str(),
        }
    }
}
...
impl LivenessRegistry {
    ...
    pub fn register(&self, spec: ComponentSpec) -> LivenessHandle {
        ...
        if let Some(old) = components.insert(spec.id, state.clone()) {
            warn!(
                "liveness component re-registered: module={} id={} old_display_name={} new_display_name={}",
                spec.id.module, spec.id.id, old.display_name, spec.display_name
            );
        }
...
impl HandleInner {
    fn update(&self, running: Option<bool>, debug: DebugInfo, transition: bool) {
        ...
        self.state
            ...
            .store(now_mono_ms, Ordering::Relaxed);
        *self.state.last_debug.lock() = debug;
    }
}
...
impl LivenessHandle {
    ...
    pub fn start(&self, debug: DebugInfo) {
        if let Some(inner) = self.0.as_ref() {
            inner.update(Some(true), debug, true);
        }
    }

    pub fn heartbeat(&self, debug: DebugInfo) {
        if let Some(inner) = self.0.as_ref() {
            inner.update(None, debug, false);
        }
    }

    pub fn stop(&self, debug: DebugInfo) {
        if let Some(inner) = self.0.as_ref() {
            inner.update(Some(false), debug, true);
        }
    }
}
...
pub struct ComponentSnapshot {
    ...
    pub last_transition_ago_ms: Option<u64>,
    pub last_debug: &'static str,
}
