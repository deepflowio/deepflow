...
impl<'a> From<&'a String> for CoreFileProcessor<'a> {
    fn from(context: &'a String) -> CoreFileProcessor<'a> {
        if context.as_bytes()[0] == '|' as u8 {
            if let Some((_, right)) = context.rsplit_once('/') {
                ...
                CoreFileProcessor::Process(context.as_str())
            }
        } else {
            CoreFileProcessor::Dir(context.as_str())
        }
    }
}
...
impl CoreFileProcessor<'_> {
    ...
    fn check_core_file_dir<P: AsRef<Path> + std::fmt::Display>(&self, core_path: P) {
        const CORE_FILE_BUFFER: usize = 256 << 10;

        let context = fs::read_dir(&core_path);
        if context.is_err() {
            warn!(
    ...
    fn check_abrt_dir<P: AsRef<Path> + std::fmt::Display>(&self, core_path: P) {
        let context = fs::read_dir(&core_path);
        if context.is_err() {
            warn!(
                "Core file read dir {} error: {}.",
...
pub fn core_file_check() {
    ...
    CoreFileProcessor::from(&path).check();
}
