...
impl Guard {
    ...
    fn check_free_disk(
        ...
        absolute_trigger_threshold: u32,
        directory: &String,
        exception_handler: &ExceptionHandler,
    ) {
        ...
        match get_disk_usage(directory) {
            Ok((total, free)) => {
                ...
                if free_percentage < percentage_trigger_threshold as f64
                    || free < (absolute_trigger_threshold as u64) << 30
                {
                ...
                if free_percentage > percentage_trigger_threshold as f64 * 1.1
                    && free as f64 > ((absolute_trigger_threshold as u64) << 30) as f64 * 1.1
                {
        ...
        let thread = thread::Builder::new().name("guard".to_owned()).spawn(move || {
            ...
            loop {
                ...
                if !in_container {
                    ...
                    &config.free_disk_circuit_breaker_directory, &exception_handler);
                }
