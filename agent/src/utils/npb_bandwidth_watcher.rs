...
impl Watcher {
    ...
    fn stop_npb(&self, reason: Reason, description: String) {
        ...
        info!("Add npb exception {}, current exceptions {}", reason, result);
        self.npb_leaky_bucket.set_rate(Some(1));
        self.traffic_count.fuse_count.fetch_add(1, Relaxed);
        self.exception_handler.set(Exception::NpbFuse);
    }
    ...
    fn try_start_npb(&self, reason: Reason) {
        ...
        info!("Npb try to reopened due to {} recovery, current exception {}", reason, self.joined_messages());
        if reopened {
