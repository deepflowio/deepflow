...
fn dispatcher_listener_callback(
    ...
    match conf.tap_mode {
        ...
        TapMode::Mirror | TapMode::Analyzer => {
            ...
            let analyzer_ip = if let Ok(ip) = config_handler
                .candidate_config
                .dispatcher
                .analyzer_ip
                .parse::<IpAddr>()
            {
                ...
                match lookup_host(&config_handler.candidate_config.dispatcher.analyzer_ip) {
                    Ok(ips) => ips[0],
                    Err(e) => {
                        warn!(
                            "Lookup dispatcher.analyzer_ip failed:{:?}, {}, deepflow-agent restart...",
                            &config_handler.candidate_config.dispatcher.analyzer_ip, e
                        );
                        thread::sleep(Duration::from_secs(1));
                        crate::utils::notify_exit(1);
                        return;
            ...
            let source_ip = match get_route_src_ip(&analyzer_ip) {
                ...
                Err(e) => {
                    ...
                    if config_handler.ctrl_ip.is_ipv6() {
            ...
            let bpf_builder = bpf::Builder {
                is_ipv6: config_handler.ctrl_ip.is_ipv6(),
                vxlan_flags: config_handler.candidate_config.yaml_config.vxlan_flags,
                npb_port: config_handler.candidate_config.yaml_config.npb_port,
                controller_port: config_handler.static_config.controller_port,
                controller_tls_port: config_handler.static_config.controller_tls_port,
                proxy_controller_port: config_handler
                    .candidate_config
                    .dispatcher
                    .proxy_controller_port,
                analyzer_source_ip: source_ip,
                analyzer_port: config_handler.candidate_config.dispatcher.analyzer_port,
            };
    ...
    fn new(
        ...
        if candidate_config.dispatcher.extra_netns_regex != "" {
            ...
            if candidate_config.tap_mode == TapMode::Local {
                ...
                // Each link will create a dispatcher_component
        ...
        {
            if candidate_config.tap_mode != TapMode::Local {
                let links = get_listener_links(&candidate_config.dispatcher);
                for l in links {
                    let if_name = l.name.clone();
                    interfaces_and_ns.push((vec![l], if_name));
                }
            } else {
                interfaces_and_ns
                    .push((get_listener_links(&candidate_config.dispatcher), "".into()));
            }
        }
        ...
        if interfaces_and_ns.is_empty() {
            for _ in 0..local_dispatcher_count {
                ...
                #[cfg(target_os = "windows")]
                interfaces_and_ns.push((vec![], "".into()));
            }
        }
...
fn build_dispatchers(
    ...
    let candidate_config = &config_handler.candidate_config;
    let yaml_config = &candidate_config.yaml_config;
    let agent_mode = config_handler.static_config.agent_mode;
    let static_config = &config_handler.static_config;
    let ctrl_ip = config_handler.ctrl_ip;
