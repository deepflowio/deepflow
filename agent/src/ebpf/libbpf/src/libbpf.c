...
int bpf_prog_load(const char *file, struct bpf_object **pobj,
		  int *prog_fd)
{
...
static inline int set_prog_type(struct bpf_program *prog,
                                struct bpf_prog_load_attr *attr)
{
        enum bpf_attach_type attach_type = 0;
        char *sec_name = strdup(prog->sec_name);

        bool is_socket = strncmp(sec_name, "socket", 6) == 0;
        bool is_kprobe = strncmp(sec_name, "kprobe/", 7) == 0;
        bool is_kretprobe = strncmp(sec_name, "kretprobe/", 10) == 0;
        bool is_tracepoint = strncmp(sec_name, "tracepoint/", 11) == 0;
        bool is_raw_tracepoint = strncmp(sec_name, "raw_tracepoint/", 15) == 0;
        bool is_xdp = strncmp(sec_name, "xdp", 3) == 0;
        bool is_perf_event = strncmp(sec_name, "perf_event", 10) == 0;
        bool is_cgroup_skb = strncmp(sec_name, "cgroup/skb", 10) == 0;
        bool is_cgroup_sk = strncmp(sec_name, "cgroup/sock", 11) == 0;
        bool is_sockops = strncmp(sec_name, "sockops", 7) == 0;
        bool is_sk_skb = strncmp(sec_name, "sk_skb", 6) == 0;
        bool is_sk_msg = strncmp(sec_name, "sk_msg", 6) == 0;

        if (!
            (is_socket || is_kprobe || is_kretprobe || is_tracepoint
             || is_raw_tracepoint || is_xdp || is_perf_event || is_cgroup_skb
             || is_cgroup_sk || is_sockops || is_sk_skb || is_sk_msg)) {
                pr_warn("sec name: %s inval\n", sec_name);
                return -EINVAL;
        }
...
int bpf_prog_load_xattr(struct bpf_prog_load_attr *attr,
			struct bpf_object **pobj, int *prog_fd)
{
	...
	bpf_object__for_each_program(prog, obj) {
                if ((err = set_prog_type(prog, attr))) {
                        bpf_object__close(obj);
                        return err;
                }
