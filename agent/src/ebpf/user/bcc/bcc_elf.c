...
int openelf_fd(int fd, Elf ** elf_out)
{
...
int openelf(const char *path, Elf ** elf_out, int *fd_out)
{
				      ...
				      const char *desc, int elf_class)
{
	...
	if (probe->semaphore && probes_shdr)
		probe->semaphore_offset =
		    probe->semaphore - probes_shdr->sh_addr +
		    probes_shdr->sh_offset;
	else
...
static int do_note_segment(Elf_Scn * section, GElf_Shdr * probes_shdr,
			   int elf_class, bcc_elf_probecb callback,
			   const char *binpath, uint64_t first_inst_offset,
			   void *payload)
{
	Elf_Data *data = NULL;
	...
	while ((data = elf_getdata(section, data)) != 0) {
		...
		while ((offset =
			gelf_getnote(data, offset, &hdr, &name_off,
				     &desc_off)) != 0) {
			...
			if (memcmp
			    ((const char *)data->d_buf + name_off, "stapsdt",
			     8) != 0)
				continue;
		      ...
		      void *payload)
{
	...
	while ((section = elf_nextscn(e, section)) != 0) {
		...
		if (name && !strcmp(name, ".note.stapsdt")) {
			GElf_Shdr *shdr_ptr =
			    found_probes_shdr ? &probes_shdr : NULL;
			if (do_note_segment
			    (section, shdr_ptr, elf_class, callback, binpath,
		       ...
		       void *payload, bool debugfile)
{
	...
	while ((data = elf_getdata(section, data)) != 0) {
		...
		for (i = 0; i < symcount; ++i) {
			...
			if (option->
			    use_symbol_type & (1 << STT_PPC64_ELFV2_SYM_LEP)) {
				/*
			...
			if (option->lazy_symbolize)
				ret =
				    callback_lazy(stridx, sym.st_name, name_len,
						  sym.st_value, sym.st_size,
						  debugfile, payload);
			else
				ret =
				    callback(name, sym.st_value, sym.st_size,
					     payload);
			if (ret < 0)
...
Elf_Data *get_section_elf_data(Elf * e, const char *section_name)
{
...
static unsigned int gnu_debuglink_crc32(unsigned int crc, char *buf, size_t len)
{
				      ...
				      int check_crc)
{
	...
	snprintf(fullpath, sizeof(fullpath), "/usr/lib/debug%s/%s", bindir,
		 name);
	if (access(fullpath, F_OK) != -1) {
		res = strdup(fullpath);
		goto DONE;
...
static char *find_debug_via_buildid(Elf * e)
{
	...
	snprintf(fullpath, sizeof(fullpath),
		 "/usr/lib/debug/.build-id/%c%c/%s.debug", buildid[0],
		 buildid[1], buildid + 2);
	if (access(fullpath, F_OK) != -1) {
...
static char *find_debug_via_debuginfod(Elf * e)
{
	...
	fd = debuginfod_find_debuginfo(client, (const unsigned char *)buildid,
				       0, &debugpath);
	if (fd >= 0)
		close(fd);
			    ...
			    int is_debug_file)
{
	...
	if (option->use_debug_file && !is_debug_file) {
		...
		if (debug_file) {
			foreach_sym_core(debug_file, callback, callback_lazy,
					 option, payload, 1);
			free(debug_file);
		}
	}
	...
	res =
	    listsymbols(e, callback, callback_lazy, payload, option,
			is_debug_file);
	elf_end(e);
	close(fd);
	return res;
...
int bcc_elf_foreach_load_section(const char *path,
				 bcc_elf_load_sectioncb callback, void *payload)
{
	Elf *e = NULL;
	int fd = -1, err = -1, res;
	size_t nhdrs, i;
...
static int bcc_free_memory_with_file(const char *path)
{
	...
	while ((section = elf_nextscn(e, section)) != 0) {
		...
		if (sh_idx == sym_shndx) {
			...
			saddr =
			    (unsigned long)bcc_free_memory - sym_addr +
			    header.sh_addr;
			...
			if (madvise
			    ((void *)saddr_n, eaddr - saddr_n, MADV_DONTNEED)) {
				fprintf(stderr,
					"madvise failed, saddr %lx, eaddr %lx\n",
					saddr, eaddr);
				goto exit;
			}
