...
{
	DECLARE_LIBBPF_OPTS(bpf_map_create_opts, p);
...
int bcc_create_map(enum bpf_map_type map_type, const char *name,
		   int key_size, int value_size, int max_entries, int map_flags)
{
...
{
	return bpf_map_lookup_batch(fd, in_batch, out_batch, keys, values,
				    count, NULL);
}
...
{
	return bpf_map_lookup_and_delete_batch(fd, in_batch, out_batch, keys,
					       values, count, NULL);
}
...
static void bpf_print_hints(int ret, char *log)
{
	...
	if (strstr(log, "unknown opcode") != NULL) {
		fprintf(stderr,
			"HINT: The 'unknown opcode' can happen if you reference "
	...
	if (helper_str != NULL) {
		...
		if (helper_id
		    && helper_id <
		    sizeof(helpers) / sizeof(struct bpf_helper)) {
			struct bpf_helper helper = helpers[helper_id - 1];
			fprintf(stderr,
				"HINT: bpf_%s missing (added in Linux %s).\n\n",
			 ...
			 unsigned long long *ptag)
{
	...
	for (i = 0; i < prog_len / 8; i++) {
		...
		if (insns[i].code == (BPF_LD | BPF_DW | BPF_IMM) &&
		    insns[i].src_reg == BPF_PSEUDO_MAP_FD && !map_ld_seen) {
			prog[i].imm = 0;
			map_ld_seen = true;
...
{
	DECLARE_LIBBPF_OPTS(bpf_prog_load_opts, p);
	...
	return bpf_prog_load(load_attr->prog_type, load_attr->name,
			     load_attr->license, load_attr->insns,
			     load_attr->insns_cnt, &p);
}
			...
			char *log_buf, unsigned log_buf_size, bool allow_rlimit)
{
	...
	if (attr->log_level > 0) {
		if (log_buf_size > 0) {
			...
			if (!tmp_log_buf) {
				fprintf(stderr,
					"bpf: Failed to allocate temporary log buffer: %s\n\n",
					strerror(errno));
				attr->log_level = 0;
			} else {
				tmp_log_buf[0] = 0;
	...
	if (name_len) {
		...
		if (attr->prog_type == BPF_PROG_TYPE_TRACING ||
			...
			ret =
			    libbpf_find_vmlinux_btf_id(attr->name + name_offset,
						       expected_attach_type);
			if (ret == -EINVAL) {
				fprintf(stderr,
					"bpf: vmlinux BTF is not found\n");
				return ret;
			} else if (ret < 0) {
				fprintf(stderr,
					"bpf: %s is not found in vmlinux BTF\n",
	...
	if (ret < 0 && attr->func_info && errno == EINVAL) {
		...
		ret =
		    libbpf_bpf_prog_load(attr, attr_log_buf, attr_log_buf_size);
	}
	...
	if (ret < 0 && errno == EPERM) {
		...
		if (getrlimit(RLIMIT_MEMLOCK, &rl) == 0) {
			...
			if (setrlimit(RLIMIT_MEMLOCK, &rl) == 0)
				ret =
				    libbpf_bpf_prog_load(attr, attr_log_buf,
							 attr_log_buf_size);
		}
	}
	...
	if (ret < 0) {
		...
		if (log_buf_size) {
			...
			if (attr->log_level == 0) {
				attr->log_level = 1;
				ret =
				    libbpf_bpf_prog_load(attr, log_buf,
							 log_buf_size);
			}
		...
		for (;;) {
			...
			tmp_log_buf[0] = 0;
			ret =
			    libbpf_bpf_prog_load(attr, tmp_log_buf,
						 tmp_log_buf_size);
			      ...
			      const char *event_type, pid_t pid, int maxactive)
{
	...
	if (write(kfd, buf, strlen(buf)) < 0) {
		if (errno == ENOENT)
			fprintf(stderr,
				"cannot attach %s, probe entry may not exist\n",
				event_type);
		else
...
static int bpf_attach_probe(int progfd, enum bpf_probe_attach_type attach_type,
			    ...
			    int maxactive, uint32_t ref_ctr_offset)
{
	int kfd, pfd = -1;
			   ...
			   int pid, int cpu, int page_cnt)
{
	...
	return bpf_open_perf_buffer_opts(raw_cb, lost_cb, cb_cookie, page_cnt,
					 &opts);
}
...
static int invalid_perf_config(uint32_t type, uint64_t config)
{
	...
	case PERF_TYPE_HARDWARE:
		if (config >= PERF_COUNT_HW_MAX) {
			fprintf(stderr,
				"HARDWARE perf event config out of range\n");
			goto is_invalid;
		}
		return 0;
	case PERF_TYPE_SOFTWARE:
		if (config >= PERF_COUNT_SW_MAX) {
			fprintf(stderr,
				"SOFTWARE perf event config out of range\n");
			goto is_invalid;
		} else if (config == 10 /* PERF_COUNT_SW_BPF_OUTPUT */ ) {
			fprintf(stderr,
				"Unable to open or attach perf event for BPF_OUTPUT\n");
			goto is_invalid;
		}
		return 0;
	case PERF_TYPE_HW_CACHE:
		if (((config >> 16) >= PERF_COUNT_HW_CACHE_RESULT_MAX) ||
			...
			fprintf(stderr,
				"HW_CACHE perf event config out of range\n");
			goto is_invalid;
		}
		return 0;
...
is_invalid:
	fprintf(stderr,
		"Invalid perf event type %" PRIu32 " config %" PRIu64 "\n",
		type, config);
	return 1;
}
...
int bpf_open_perf_event(uint32_t type, uint64_t config, int pid, int cpu)
{
	...
	fd = syscall(__NR_perf_event_open, &attr, pid, cpu, -1,
		     PERF_FLAG_FD_CLOEXEC);
	if (fd < 0) {
		fprintf(stderr, "perf_event_open: %s\n", strerror(errno));
		return -1;
...
{
	int fd =
	    syscall(__NR_perf_event_open, perf_event_attr, pid, cpu, group_fd,
