...
static char *_procutils_memfd_path(const int pid, const uint64_t inum)
{
	...
	while (path == NULL && (dent = readdir(dirstream)) != NULL) {
		snprintf(path_buffer, (PATH_MAX + 1), "%s/%s", dirstr,
			 dent->d_name);
			     ...
			     bcc_procutils_modulecb callback, void *payload)
{
	...
	while (true) {
		...
		if (fscanf(procmap, "%lx-%lx %4s %llx %lx:%lx %lu%[^\n]",
			   &mod.start_addr, &mod.end_addr, perm,
			   &mod.file_offset, &mod.dev_major, &mod.dev_minor,
			   &mod.inode, buf) != 8)
			break;
		...
		if (strstr(name, "/memfd:")) {
			char *memfd_name =
			    _procutils_memfd_path(pid, mod.inode);
			if (memfd_name != NULL) {
...
const char *bcc_procutils_language(int pid)
{
	...
	snprintf(procfilename, sizeof(procfilename), "/proc/%ld/exe",
		 (long)pid);
	...
	snprintf(procfilename, sizeof(procfilename), "/proc/%ld/maps",
		 (long)pid);
	...
	do {
		...
		ret =
		    fscanf(procfile, "%llx-%llx %s %llx %s %lld", &begin, &end,
			   perm, &size, dev, &inode);
		...
		if (ret == 6) {
			...
			while (isspace(mapname[0]))
				mapname++;
			for (i = 0; i < nb_languages; i++) {
				snprintf(pathname, sizeof(pathname), "/lib%s",
					 languages[i]);
