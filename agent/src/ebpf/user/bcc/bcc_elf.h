...
typedef void (*bcc_elf_probecb) (const char *,
				 const struct bcc_elf_usdt *, void *);
...
typedef int (*bcc_elf_symcb_lazy) (size_t, size_t, size_t, uint64_t,
				   uint64_t, int, void *);
// Segment virtual address, memory size, file offset, payload
// Callback returning a negative value indicates to stop the iteration
typedef int (*bcc_elf_load_sectioncb) (uint64_t, uint64_t, uint64_t,
				       void *);
...
int bcc_elf_foreach_sym_lazy(const char *path,
			     bcc_elf_symcb_lazy callback, void *option,
			     void *payload);
