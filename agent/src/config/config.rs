...
impl From<Matcher> for KeywordMatcher {
    fn from(value: Matcher) -> KeywordMatcher {
        let (match_keyword_bytes, match_type) = match value.match_type.as_str() {
            "string" => (
                value.match_keyword.into_bytes().to_vec(),
    ...
    pub fn get_extra_field_policies(
            ...
            .custom_field_policies
        {
            ...
            if let (Some(extra_policy), Some(segment_map)) = (
                ...
                // default 0 index for all ports, even is empty value
        ...
        fn add_extra_policy(
            ...
            if custom_field_policy.port_list.is_empty() {
                custom_field_policy.merge_into(&mut extra_policy[POLICY_INDEX_OF_ALL_PORT]);
            } else {
                ...
                for (start, end) in port_pairs {
                    // no matter what should check none port for all policy
                    segment_map.add_range(
                        start,
                        end,
                        vec![index_of_policy, POLICY_INDEX_OF_ALL_PORT],
                    );
                }
            }
        }
