    ...
    D: Deserializer<'de>,
{
    match String::deserialize(deserializer)?.to_lowercase().as_str() {
        "string" => Ok(MatchType::String),
        "istring" => Ok(MatchType::Istring),
...
if #[cfg(feature = "enterprise")] {
        impl From<&Field> for ExtraField {
            fn from(v: &Field) -> ExtraField {
                ExtraField {
                    field_match_type: v.field_match_type,
                    ...
                    metric_type: v.metric_type.clone(),
                }
            }
        }
        impl From<Matcher> for KeywordMatcher {
            fn from(value: Matcher) -> KeywordMatcher {
                let match_keyword_bytes = match value.match_type {
                    MatchType::String | MatchType::Istring => value.match_keyword.as_bytes().to_vec(),
