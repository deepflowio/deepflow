...
if #[cfg(feature = "enterprise")] {
        use public::{
            ...
            segment_map::{parse_u16_range_list_to_port_pairs, SegmentMap},
        };
...
impl From<&CustomFieldPolicy> for ExtraCustomFieldPolicy {
    fn from(v: &CustomFieldPolicy) -> ExtraCustomFieldPolicy {
        ...
        v.merge_into(&mut extra_field_policy);
        extra_field_policy
    }
}
...
impl CustomFieldPolicy {
    fn merge_into(&self, other: &mut ExtraCustomFieldPolicy) {
...
impl UserConfig {
    ...
    pub fn get_custom_protocol_config(&self) -> ExtraCustomProtocolConfig {
        let custom_protocol_config = &self
            ...
            .application_protocol_inference
            .custom_protocols;
        let mut port_segmentmap = SegmentMap::new();
        let mut protocol_characters = Vec::new();
        let mut port_protocol_characters = Vec::new();
        for c in custom_protocol_config {
            if c.pre_filter.port_list.is_empty() {
                ...
                port_protocol_characters.push(ExtraProtocolCharacters::from(c));
                let index_of_characters = port_protocol_characters.len() - 1;
                for (start, end) in port_pairs {
                    port_segmentmap.add_range(start, end, index_of_characters);
                }
            }
        }
        port_segmentmap.merge_segments();
        ExtraCustomProtocolConfig {
            port_segmentmap,
            protocol_characters,
            port_protocol_characters,
        }
    }

    #[cfg(feature = "enterprise")]
    pub fn get_extra_field_policies(
        &self,
    ) -> (
        HashMap<L7ProtocolEnum, SegmentMap<usize>>,
        HashMap<L7ProtocolEnum, Vec<ExtraCustomFieldPolicy>>,
    ) {
        let mut extra_policies: HashMap<L7ProtocolEnum, Vec<ExtraCustomFieldPolicy>> =
            HashMap::new();
        let mut extra_segment_map = HashMap::new();
            ...
            .custom_field_policies
        {
            ...
            if let (Some(extra_policy), Some(segment_map)) = (
                extra_policies.get_mut(&l7_protocol),
                extra_segment_map.get_mut(&l7_protocol),
            ) {
                add_extra_policy(&p, extra_policy, segment_map);
            } else {
                let mut extra_policy = vec![ExtraCustomFieldPolicy::default()];
                let mut segment_map = SegmentMap::new();
                add_extra_policy(&p, &mut extra_policy, &mut segment_map);
                extra_policies.insert(l7_protocol.clone(), extra_policy);
                extra_segment_map.insert(l7_protocol, segment_map);
            }
        }

        for (_, segment_map) in extra_segment_map.iter_mut() {
            // push INDEX_OF_ALL_PORTS to ensuring all policies will be checked
            // 对所有 segment 确保必定要检查 ALL PORTS 策略
            segment_map.map(|v| v.value.push(INDEX_OF_ALL_PORTS));
            // incase none config was found
            segment_map.add_range(u16::MIN, u16::MAX, INDEX_OF_ALL_PORTS);
            segment_map.merge_segments();
        }

        #[inline]
        fn add_extra_policy(
            custom_field_policy: &CustomFieldPolicy,
            extra_policy: &mut Vec<ExtraCustomFieldPolicy>,
            segment_map: &mut SegmentMap<usize>,
        ) {
            if custom_field_policy.port_list.is_empty() {
                custom_field_policy.merge_into(&mut extra_policy[INDEX_OF_ALL_PORTS]);
            } else {
                ...
                extra_policy.push(ExtraCustomFieldPolicy::from(custom_field_policy));
                let index_of_policy = extra_policy.len() - 1;
                for (start, end) in port_pairs {
                    segment_map.add_range(start, end, index_of_policy);
                }
            }
        }
        (extra_segment_map, extra_policies)
    }
