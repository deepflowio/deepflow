...
fn test_reassemble() {
    ...
    for (t, p) in packets.iter() {
        let _ = buf.reassemble(t.seq, p.as_slice(), 0, 0);
    }
...
fn test_bad_frame() {
    ...
    match buf
        .reassemble(before_base_frame.seq, p.as_slice(), 0, 0)
        .unwrap_err()
    {
    ...
    match buf
        .reassemble(bad_frame.seq, p.as_slice(), 0, 0)
        .unwrap_err()
    {
...
fn test_frame_exceed() {
    ...
    match buf.reassemble(next_fram.0.seq, next_fram.1.as_slice(), 0, 0) {
        Ok(_) => unreachable!(),
        Err(e) => match e {
...
fn test_payload_exceed() {
    ...
    let _ = buf.reassemble(f0.0.seq, f0.1.as_slice(), 0, 0);
    let _ = buf.reassemble(f1.0.seq, f1.1.as_slice(), 0, 0);
    ...
    match buf.reassemble(exceed_frame.0.seq, exceed_frame.1.as_slice(), 0, 0) {
        Ok(_) => unreachable!(),
        Err(e) => match e {
            TcpReassembleError::BufferFlush(mut f_buf) => {
