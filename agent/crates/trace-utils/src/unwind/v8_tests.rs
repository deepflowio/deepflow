...
mod tests {
    ...
    fn test_v8_unwind_table_operations() {
        ...
        let table = V8UnwindTable::new(-1, -1); // Mock file descriptors
        assert!(table.is_ok(), "V8UnwindTable creation should succeed");
    ...
    fn test_v8_stack_merging() {
        ...
        let result_len = unsafe {
            merge_v8_stacks(
                ...
                buffer.len(),
                std::ffi::CString::new(js_stack).unwrap().as_ptr() as *const std::ffi::c_void,
                std::ffi::CString::new(native_stack).unwrap().as_ptr() as *const std::ffi::c_void,
            )
        };
    ...
    fn test_concurrent_operations() {
        ...
        let table = Arc::new(std::sync::Mutex::new(V8UnwindTable::new(-1, -1).unwrap()));
        let mut handles = vec![];
