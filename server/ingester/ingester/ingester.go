...
import (
	...
	rozecfg "github.com/deepflowio/deepflow/server/ingester/roze/config"
	"github.com/deepflowio/deepflow/server/ingester/roze/roze"
	streamcfg "github.com/deepflowio/deepflow/server/ingester/stream/config"
	"github.com/deepflowio/deepflow/server/ingester/stream/stream"
)
...
func Start(configPath string, shared *servercommon.ControllerIngesterShared) []io.Closer {
	...
	if cfg.StreamRozeEnabled {
		...
		// 写遥测数据
		roze, err := roze.NewRoze(rozeConfig, receiver, platformDataManager)
		checkError(err)
		roze.Start()
		closers = append(closers, roze)

		// 写流日志数据
		stream, err := stream.NewStream(streamConfig, receiver, platformDataManager)
		checkError(err)
		stream.Start()
