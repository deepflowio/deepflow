...
func (d *Decoder) WriteAgentLog(agentId uint16, bs []byte) error {
	...
	if agentId != 0 {
		...
		s.AutoInstanceID, s.AutoInstanceType = ingestercommon.GetAutoInstance(s.PodID, 0, s.PodNodeID, s.PodGroupID, s.L3DeviceID, uint32(s.SubnetID), uint8(s.L3DeviceType), podGroupType, s.L3EpcID)
