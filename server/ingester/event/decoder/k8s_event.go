...
func (d *Decoder) WriteK8sEvent(vtapId uint16, e *pb.KubernetesEvent) {
	...
	s.AutoInstanceID, s.AutoInstanceType = ingestercommon.GetAutoInstance(s.PodID, s.GProcessID, s.PodNodeID, s.PodGroupID, s.L3DeviceID, uint32(s.SubnetID), uint8(s.L3DeviceType), podGroupType, s.L3EpcID)
