...
func (d *Decoder) WritePerfEvent(vtapId uint16, e *pb.ProcEvent) {
	...
	if e.IoEventData != nil {
		...
		s.EventDescription = fmt.Sprintf("process %s (%d) %s %d bytes and took %.3fms", string(e.ProcessKname), e.Pid, s.EventType, ioData.BytesCount, float64(ioData.Latency)/float64(time.Millisecond))
