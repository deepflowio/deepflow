...
import (
	...
	"github.com/deepflowys/deepflow/server/ingester/flow_log/config"
	"github.com/deepflowys/deepflow/server/ingester/flow_log/dbwriter"
	"github.com/deepflowys/deepflow/server/ingester/flow_log/decoder"
	"github.com/deepflowys/deepflow/server/ingester/flow_log/geo"
	"github.com/deepflowys/deepflow/server/ingester/flow_log/throttler"
...
func NewFlowLog(config *config.Config, recv *receiver.Receiver) (*FlowLog, error) {
...
func NewLogger(msgType datatype.MessageType, config *config.Config, controllers []net.IP, manager *dropletqueue.Manager, recv *receiver.Receiver, flowLogWriter *dbwriter.FlowLogWriter, flowLogId common.FlowLogID, flowTagWriter *flow_tag.FlowTagWriter) *Logger {
	...
	for i := 0; i < queueCount; i++ {
		...
		if controllers != nil {
			platformDatas[i] = grpc.NewPlatformInfoTable(controllers, int(config.Base.ControllerPort), config.Base.GrpcBufferSize, config.Base.ServiceLabelerLruCap, "flow-log-"+datatype.MessageTypeString[msgType]+"-"+strconv.Itoa(i), "", config.Base.NodeIP, nil)
			if i == 0 {
...
func NewL4FlowLogger(config *config.Config, controllers []net.IP, manager *dropletqueue.Manager, recv *receiver.Receiver, flowLogWriter *dbwriter.FlowLogWriter) *Logger {
	...
	for i := 0; i < queueCount; i++ {
		...
		platformDatas[i] = grpc.NewPlatformInfoTable(controllers, int(config.Base.ControllerPort), config.Base.GrpcBufferSize, config.Base.ServiceLabelerLruCap, "l4-flow-log-"+strconv.Itoa(i), "", config.Base.NodeIP, nil)
		if i == 0 {
			debug.ServerRegisterSimple(CMD_PLATFORMDATA, platformDatas[i])
		}
		decoders[i] = decoder.NewDecoder(
...
func NewL7FlowLogger(config *config.Config, controllers []net.IP, manager *dropletqueue.Manager, recv *receiver.Receiver, flowLogWriter *dbwriter.FlowLogWriter, flowTagWriter *flow_tag.FlowTagWriter) *Logger {
	...
	for i := 0; i < queueCount; i++ {
		...
		platformDatas[i] = grpc.NewPlatformInfoTable(controllers, int(config.Base.ControllerPort), config.Base.GrpcBufferSize, config.Base.ServiceLabelerLruCap, "l7-flow-log-"+strconv.Itoa(i), "", config.Base.NodeIP, nil)
		decoders[i] = decoder.NewDecoder(
			i,
			msgType,
			platformDatas[i],
