...
const (
	zstdCompressFlag uint8 = 0x1
	gzipCompressFlag uint8 = 0x2
)
...
func (d *Decoder) handleProfileData(vtapID uint16, decoder *codec.SimpleDecoder) {
	for !decoder.IsEnd() {
		...
		case "jfr":
			...
			compressFlag := gzipCompressFlag
			if profile.DataCompressed == 1 {
				compressFlag &= zstdCompressFlag
			}
...
func (d *Decoder) decompressData(data []byte, compressFlag uint8) []byte {
	// zstdCompress comes from sender
	if compressFlag|zstdCompressFlag == zstdCompressFlag {
		var err error
		data, err = profile_common.ZstdDecompress(d.decompressBuffer[:0], data)
		if err != nil {
			log.Errorf("decompress profile data failed, decompressType=%d, len=%d, err=%s", zstdCompressFlag, len(data), err)
			return data
		}
	}
	// gzipCompress comes from application-profiler
	if compressFlag|gzipCompressFlag == gzipCompressFlag {
		var err error
