...
func (i *Issu) addColumnDatasource(index int, d *DatasourceInfo, isMapTable, isAppTable, isNetworkTable bool) ([]*ColumnAdd, error) {
	...
	rawTable := flow_metrics.GetMetricsTables(ckdb.MergeTree, common.CK_VERSION, ckdb.DF_CLUSTER, ckdb.DF_STORAGE_POLICY, i.ckdbType, 7, 1, 7, 1, i.cfg.GetCKDBColdStorages())[flow_metrics.MetricsTableNameToID(d.name[:lastDotIndex+1]+"1m")] // base table always is '1m'
	// create table mv
...
func (i *Issu) DropTable(db, table string) {
	for idx, connect := range i.Connections {
		...
		if strings.Compare(oldVersion, "v6.6.9.5") >= 0 || oldVersion == "" {
			continue
		}
