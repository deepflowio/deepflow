...
func NewPodGroup(fpermitCfg config.FPermit) *PodGroup {
	g := new(PodGroup)
...
func (p *PodGroup) conditionsMapToStruct(fcs common.FilterConditions) filter.Condition {
	c := filter.NewAND()
	c.Init(fcs)
	return c
}

func (p *PodGroup) userPermittedResourceToConditions(upr *UserPermittedResource) (common.FilterConditions, bool) {
	fc := &model.PodGroupFilterConditions{
		VPCIDs:          upr.VPCIDs,
