...
func NewLB(fpermitCfg config.FPermit) *LB {
	g := new(LB)
...
func (p *LB) conditionsMapToStruct(fcs common.FilterConditions) filter.Condition {
	c := filter.NewAND()
	c.Init(fcs)
	return c
}

func (p *LB) userPermittedResourceToConditions(upr *UserPermittedResource) (common.FilterConditions, bool) {
	fcs := &model.LBFilterConditions{
		VPCIDs: upr.VPCIDs,
	}
