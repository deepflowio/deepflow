...
func NewPodIngress(fpermitCfg config.FPermit) *PodIngress {
	g := new(PodIngress)
...
func (p *PodIngress) conditionsMapToStruct(fcs common.FilterConditions) filter.Condition {
	c := filter.NewAND()
	c.Init(fcs)
	return c
}

func (p *PodIngress) userPermittedResourceToConditions(upr *UserPermittedResource) (common.FilterConditions, bool) {
	fc := &model.PodIngressFilterConditions{
		VPCIDs:          upr.VPCIDs,
