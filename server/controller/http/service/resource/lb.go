...
func NewLBGet(urlInfo *model.URLInfo, userInfo *model.UserInfo, fpermitCfg config.FPermit) *LB {
	log.Infof("request info: %#v, %#v", urlInfo, userInfo)
	s := &LB{newServiceGet(
		ctrlrcommon.RESOURCE_TYPE_LB_EN,
		data.GetDataProvider(ctrlrcommon.RESOURCE_TYPE_LB_EN, &data.RequiredConfigs{}),
	)}
	s.generateDataContext(urlInfo, userInfo, generator.NewLB(fpermitCfg))
	return s
}
