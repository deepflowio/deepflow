...
func GetCMDAndNamespace(orgID, agentID int) (*model.RemoteExecResp, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w agent(name: %s, key: %s) remote exec map not found",
			common.ERR_NO_AGENT_REMOTE_EXEC_CONNECT, agent.Name, key)
	}
	manager.ResetResp()
	...
	for {
		...
		default:
			if len(manager.GetCommands()) != 0 && len(manager.GetNamespaces()) != 0 {
				log.Infof("weiqiang len(manager.GetCommands())=%d, len(manager.GetNamespaces())=%d",
					len(manager.GetCommands()), len(manager.GetNamespaces()))
...
func RunAgentCMD(orgID, agentID int, req *trident.RemoteExecRequest) (string, error) {
	...
	manager, ok := AgentRemoteExecMap[key]
	if !ok {
		return "", fmt.Errorf("%w agent(name: %s, key: %s) remote exec map not found",
			common.ERR_NO_AGENT_REMOTE_EXEC_CONNECT, agent.Name, key)
	}
	manager.ResetResp()
	manager.ExecCH <- req
