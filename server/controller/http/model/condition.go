...
// 直接应用到 http response 的过滤条件
type VMFilterConditions struct {
	Convertor

	Lcuuids          []string `schema:"lcuuid,omitempty" json:"LCUUID,omitempty"` // TODO no need json tag
...
// query string 中的过滤条件
// 其中用户过滤条件不可以直接应用到 http response，需要转换成对应的过滤条件
...
// pod filter condition directly applied to pod http response model
...
// pod filter condition formed by http request query
