...
func NewPodReplicaSet(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodReplicaSet {
	return &PodReplicaSet{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodReplicaSet) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_REPLICA_SET, p.Get)
}

func (p *PodReplicaSet) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodReplicaSetQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodReplicaSetGet(
		NewURLInfo(
