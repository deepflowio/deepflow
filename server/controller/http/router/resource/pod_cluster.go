...
func NewPodCluster(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodCluster {
	return &PodCluster{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodCluster) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_CLUSTER, p.Get)
}

func (p *PodCluster) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodClusterQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodClusterGet(
		NewURLInfo(
...
func (p *PodCluster) Update(c *gin.Context) {
