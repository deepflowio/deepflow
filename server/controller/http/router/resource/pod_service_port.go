...
func NewPodServicePort(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodServicePort {
	return &PodServicePort{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodServicePort) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_SERVICE_PORT, p.Get)
}

func (p *PodServicePort) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodServicePortQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodServicePortGet(
		NewURLInfo(
...
func (p *PodServicePort) Update(c *gin.Context) {
