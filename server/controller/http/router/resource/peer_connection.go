...
func NewPeerConnection(hCfg httpCfg.Config, fCfg config.FPermit) *PeerConnection {
	return &PeerConnection{httpCfg: hCfg, fpermitCfg: fCfg}
}

func (p *PeerConnection) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_PEER_CONNECTION, p.Get)
}

func (p *PeerConnection) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PeerConnectionQuery](c.Request.URL.Query())

	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPeerConnectionGet(
		NewURLInfo(
