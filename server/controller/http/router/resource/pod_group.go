...
func NewPodGroup(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodGroup {
	return &PodGroup{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodGroup) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_GROUP, p.Get)
}

func (p *PodGroup) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodGroupQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodGroupGet(
		NewURLInfo(
...
func (p *PodGroup) Update(c *gin.Context) {
