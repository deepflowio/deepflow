...
func NewPodNode(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodNode {
	return &PodNode{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodNode) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_NODE, p.Get)
}

func (p *PodNode) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodNodeQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodNodeGet(
		NewURLInfo(
...
func (p *PodNode) Update(c *gin.Context) {
