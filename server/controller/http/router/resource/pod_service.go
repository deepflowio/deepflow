...
func NewPodService(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodService {
	return &PodService{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodService) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_SERVICE, p.Get)
}

func (p *PodService) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodServiceQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodServiceGet(
		NewURLInfo(
...
func (p *PodService) Update(c *gin.Context) {
