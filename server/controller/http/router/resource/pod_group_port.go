...
func NewPodGroupPort(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodGroupPort {
	return &PodGroupPort{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodGroupPort) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_GROUP_PORT, p.Get)
}

func (p *PodGroupPort) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodGroupPortQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodGroupPortGet(
		NewURLInfo(
