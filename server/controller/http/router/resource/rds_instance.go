...
func NewRDSInstance(hCfg httpCfg.Config, fCfg config.FPermit) *RDSInstance {
	return &RDSInstance{httpCfg: hCfg, fpermitCfg: fCfg}
}

func (p *RDSInstance) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_RDS_INSTANCE, p.Get)
}

func (p *RDSInstance) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.RDSInstanceQuery](c.Request.URL.Query())

	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewRDSInstanceGet(
		NewURLInfo(
