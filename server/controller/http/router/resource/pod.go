...
func NewPod(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *Pod {
	return &Pod{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *Pod) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD, p.Get)
}

func (p *Pod) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodGet(
		NewURLInfo(
