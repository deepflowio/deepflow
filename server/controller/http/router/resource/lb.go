...
func NewLB(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *LB {
	return &LB{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *LB) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_LB, p.Get)
}

func (p *LB) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.LBQuery](c.Request.URL.Query())

	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewLBGet(
		NewURLInfo(
