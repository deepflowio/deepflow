...
func NewPodIngress(hCfg httpCfg.Config, rCfg dbredis.Config, fCfg config.FPermit) *PodIngress {
	return &PodIngress{httpCfg: hCfg, redisCfg: rCfg, fpermitCfg: fCfg}
}

func (p *PodIngress) RegisterTo(ge *gin.Engine) {
	ge.GET(httpcommon.PATH_POD_INGRESS, p.Get)
}

func (p *PodIngress) Get(c *gin.Context) {
	header := NewHeaderValidator(c.Request.Header, p.fpermitCfg)
	query := NewQueryValidator[model.PodIngressQuery](c.Request.URL.Query())
	if err := NewValidators(header, query).Validate(); err != nil {
		common.BadRequestResponse(c, httpcommon.INVALID_PARAMETERS, err.Error())
		return
	}
	service := resource.NewPodIngressGet(
		NewURLInfo(
...
func (p *PodIngress) Update(c *gin.Context) {
