...
func (m *Allocator) Init(ctx context.Context, cfg *prometheuscfg.Config) {
	...
	m.IDPoolMap = map[string]idPoolUpdater{
		ResourcePrometheusMetricName: &idPool[mysql.PrometheusMetricName]{ResourceType: ResourcePrometheusMetricName, Max: cfg.ResourceMaxID0},
		ResourcePrometheusLabelName:  &idPool[mysql.PrometheusLabelName]{ResourceType: ResourcePrometheusLabelName, Max: cfg.ResourceMaxID0},
		ResourcePrometheusLabelValue: &idPool[mysql.PrometheusLabelValue]{ResourceType: ResourcePrometheusLabelValue, Max: cfg.ResourceMaxID1},
	}
	m.RefreshInterval = cfg.IDPoolRefreshInterval
	return
}
