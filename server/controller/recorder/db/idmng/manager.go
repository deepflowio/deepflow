...
func newIDManager(cfg RecorderConfig, orgID int) (*IDManager, error) {
	...
	mng.resourceTypeToIDPool = map[string]IDPoolUpdater{
		...
		ctrlrcommon.RESOURCE_TYPE_GPROCESS_EN:        newIDPool[metadbmodel.Process](mng.org, ctrlrcommon.RESOURCE_TYPE_GPROCESS_EN, cfg.ResourceMaxID1),
	...
	if orgTableExists && orgID == ctrlrcommon.DEFAULT_ORG_ID {
		...
		mng.resourceTypeToIDPool[ctrlrcommon.RESOURCE_TYPE_ORG_EN].(*IDPool[metadbmodel.ORG]).resetKeyField("org_id")
	}

	mng.resourceTypeToIDPool[ctrlrcommon.RESOURCE_TYPE_GPROCESS_EN].(*IDPool[metadbmodel.Process]).resetKeyField("gid")
	return mng, nil
}
