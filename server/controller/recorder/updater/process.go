...
func NewProcess(wholeCache *cache.Cache, cloudData []cloudmodel.Process) *Process {
	...
	updater.hookers[hookerAddPage] = updater
	updater.hookers[hookerDeletePage] = updater
	return updater
}
...
func (p *Process) beforeAddPage(dbData []*metadbmodel.Process) ([]*metadbmodel.Process, interface{}, bool) {
	...
	if len(identifierToNewGID) > 0 {
		...
		ids, err := idmng.GetIDs(p.metadata.GetORGID(), p.resourceType, len(identifierToNewGID))
		if err != nil {
			log.Errorf("%s request ids failed", p.resourceType, p.metadata.LogPrefixes)
			return dbData, nil, false
		}
		log.Infof("%s use ids: %v", p.resourceType, ids, p.metadata.LogPrefixes)
...
func (p *Process) afterDeletePage(dbData []*metadbmodel.Process) (interface{}, bool) {
	...
	err := idmng.ReleaseIDs(p.metadata.GetORGID(), ctrlrcommon.RESOURCE_TYPE_GPROCESS_EN, gids)
	if err != nil {
		log.Errorf("%s release ids: %v failed", p.resourceType, gids, p.metadata.LogPrefixes)
	}
	log.Infof("%s return used ids: %v", p.resourceType, gids, p.metadata.LogPrefixes)
