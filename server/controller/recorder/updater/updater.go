...
func (u *UpdaterBase[CT, BT, MPT, MT, MAPT, MAT, MAAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT, MDAT]) addPage(dbItemsToAdd []*MT) {
	var addition *MAAT
	if hooker, ok := u.hookers[hookerAddPage].(addPageHooker[MT, MAAT]); ok {
		dbItemsToAdd, addition, ok = hooker.beforeAddPage(dbItemsToAdd)
		if !ok {
...
func (u *UpdaterBase[CT, BT, MPT, MT, MAPT, MAT, MAAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT, MDAT]) deletePage(lcuuids []string) {
	if dbItems, ok := u.dbOperator.DeleteBatch(lcuuids); ok {
		...
		var addition *MDAT
		if hooker, ok := u.hookers[hookerDeletePage].(deletePageHooker[MT, MDAT]); ok {
			addition, ok = hooker.afterDeletePage(dbItems)
			if !ok {
