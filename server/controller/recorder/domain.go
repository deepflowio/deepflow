...
func (d *domain) shouldRefresh(cloudData cloudmodel.Resource) error {
	if cloudData.Verified {
		// 这两种云平台可能存在此场景：没有 network 数据，但是有属于 public network 的 vinterface 数据
		if ((d.metadata.GetDomainInfo().Type != common.CLOUD_TOWER && d.metadata.GetDomainInfo().Type != common.FUSIONCOMPUTE) && len(cloudData.Networks) == 0) || len(cloudData.VInterfaces) == 0 {
