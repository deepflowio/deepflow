...
func NewCacheManager(ctx context.Context, cfg config.RecorderConfig, md *rcommon.Metadata) *CacheManager {
	...
	log.Infof("new cache manager for %d sub domains", len(subDomains), mng.metadata.LogPrefixes)
...
func (c *Cache) refreshNetworks() []int {
	...
	networkIDs := []int{}
	networks, err := rcommon.PageWhereFind[mysqlmodel.Network](c.metadata, c.getConditionDomainSubDomain())
	if err != nil {
		c.refreshFailed = true
...
func (c *Cache) refreshVInterfaces() {
	...
	vifs, err := rcommon.PageWhereFind[mysqlmodel.VInterface](c.metadata, c.getConditionDomainSubDomain())
	if err != nil {
		c.refreshFailed = true
...
func (c *Cache) refreshWANIPs() {
	...
	wanIPs, err := rcommon.PageWhereFind[mysqlmodel.WANIP](c.metadata, c.getConditionDomainSubDomain())
	if err != nil {
		c.refreshFailed = true
...
func (c *Cache) refreshLANIPs() {
	...
	lanIPs, err := rcommon.PageWhereFind[mysqlmodel.LANIP](c.metadata, c.getConditionDomainSubDomain())
	if err != nil {
		c.refreshFailed = true
