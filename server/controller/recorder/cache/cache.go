...
func (c *Cache) refreshConfigMaps() {
	...
	configMaps, err := rcommon.PageWhereFind[metadbmodel.ConfigMap](c.metadata, "domain = ? AND (sub_domain = ? OR sub_domain IS NULL)", c.metadata.Domain.Lcuuid, c.metadata.SubDomain.Lcuuid)
	if err != nil {
...
func (c *Cache) refreshProcesses() {
	...
	processes, err := query.FindInBatches[metadbmodel.Process](c.metadata.DB.Unscoped().Where("domain = ? AND (sub_domain = ? OR sub_domain IS NULL)", c.metadata.Domain.Lcuuid, c.metadata.SubDomain.Lcuuid))
	if err != nil {
