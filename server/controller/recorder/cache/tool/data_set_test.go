...
func (t *SuiteTest) TestAddNetworkInTDS() {
	id := RandID()
	name := RandName()
	dbItem := &metadbmodel.Network{Base: metadbmodel.Base{ID: id, Lcuuid: RandLcuuid()}, Name: name}
	ds := NewDataSet()
...
func (t *SuiteTest) TestAddWANIPInTDS() {
	...
	vifID := RandID()
	ip := faker.IPv4()
	dbItem := &metadbmodel.WANIP{Base: metadbmodel.Base{ID: RandID(), Lcuuid: lcuuid}, VInterfaceID: vifID, IP: ip}
	ds := NewDataSet()
	ds.AddWANIP(dbItem)
...
func (t *SuiteTest) TestGetNetworkNameByID() {
	...
	id2 := RandID()
	name2 := RandName()
	dbItem := &metadbmodel.Network{Base: metadbmodel.Base{ID: id2, Lcuuid: RandLcuuid()}, Name: name2}
	metadb.DefaultDB.Create(&dbItem)
...
func (t *SuiteTest) TestGetVInterfaceLcuuidByID() {
	...
	id2 := RandID()
	lcuuid2 := RandLcuuid()
	dbItem := &metadbmodel.VInterface{Base: metadbmodel.Base{ID: id2, Lcuuid: lcuuid2}}
	metadb.DefaultDB.Create(&dbItem)
...
func (t *SuiteTest) TestGetVInterfaceIDByWANIPLcuuid() {
	...
	vifID2 := RandID()
	lcuuid2 := RandLcuuid()
	dbItem := &metadbmodel.WANIP{Base: metadbmodel.Base{Lcuuid: lcuuid2}, VInterfaceID: vifID2}
	metadb.DefaultDB.Create(&dbItem)
...
func (t *SuiteTest) TestGetWANIPByLcuuid() {
	...
	ip2 := faker.IPv4()
	lcuuid2 := RandLcuuid()
	dbItem := &metadbmodel.WANIP{Base: metadbmodel.Base{Lcuuid: lcuuid2}, IP: ip2}
	metadb.DefaultDB.Create(&dbItem)
