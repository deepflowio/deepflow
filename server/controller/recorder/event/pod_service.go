...
type PodService struct {
	EventManager[cloudmodel.IP, mysql.PodService, *cache.PodService]
}
...
func (p *PodService) ProduceByAdd(items []*mysql.PodService) {
	for _, item := range items {
		p.createAndPutEvent(eventapi.RESOURCE_EVENT_TYPE_CREATE, common.VIF_DEVICE_TYPE_POD_SERVICE, item.ID, item.Name, "")
	}
}
...
func (p *PodService) ProduceByDelete(lcuuids []string) {
	for _, lcuuid := range lcuuids {
		...
		p.createAndPutEvent(eventapi.RESOURCE_EVENT_TYPE_DELETE, common.VIF_DEVICE_TYPE_POD_SERVICE, id, name, "")
	}
}
