...
func (i *LANIP) ProduceByAdd(items []*mysql.LANIP) {
	for _, item := range items {
		...
		if ok {
			...
			log.Error(i.metadata.Logf("%s lcuuid (id: %d) for %s not found",
...
func (i *LANIP) ProduceByDelete(lcuuids []string) {
	for _, lcuuid := range lcuuids {
		...
		if ok {
			...
			log.Error(i.metadata.Logf("%s id for %s (lcuuid: %s) not found",
		...
		if !ok {
			log.Error(i.metadata.Logf("%s (lcuuid: %s) ip not found", ctrlrcommon.RESOURCE_TYPE_LAN_IP_EN, lcuuid))
		}
