...
func (r *WholeSubDomain) OnAnyChanged(md *message.Metadata) {
	...
	for _, item := range dbItems {
		...
		log.Infof("TODO resource event_type: %s, event: %#v", event.Type, event, md.LogPrefixes)
		if event.Type == eventapi.RESOURCE_EVENT_TYPE_RECREATE {
			...
			log.Infof("TODO pod group ids: %v", md.GetToolDataSet().GetPodGroupIDsByConfigMapID(int(event.ConfigMapID)), md.LogPrefixes)
