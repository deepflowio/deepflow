...
var agentStateCounter = NewAgentStateCounter()
...
func AddAgentStateCounters(orgID int, vtaps ...*model.VTap) {
	agentStateCounter.AddAgentStateCounters(orgID, vtaps...)
}
...
func Start() {
	...
	err = stats.RegisterCountableWithModulePrefix("controller_", "agent", agentStateCounter, stats.OptionStatTags{"type": "agent"})
	if err != nil {
		log.Error(err)
	}
}
