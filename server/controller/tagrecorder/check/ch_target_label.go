...
type ChTargetLabel struct {
	UpdaterBase[metadbmodel.ChTargetLabel, PrometheusTargetLabelKey]
}
...
func (l *ChTargetLabel) generateNewData() (map[PrometheusTargetLabelKey]metadbmodel.ChTargetLabel, bool) {
	var prometheusMetricNames []metadbmodel.PrometheusMetricName

	err := metadb.DefaultDB.Unscoped().Find(&prometheusMetricNames).Error
	if err != nil {
...
func (l *ChTargetLabel) generateMetricTargetData() (map[string][]int, bool) {
	...
	err := metadb.DefaultDB.Unscoped().Find(&prometheusMetricTargets).Error
