...
type ChPrometheusLabelName struct {
	UpdaterBase[metadbmodel.ChPrometheusLabelName, IDKey]
}
...
func (l *ChPrometheusLabelName) generateNewData() (map[IDKey]metadbmodel.ChPrometheusLabelName, bool) {
	var prometheusLabelName []metadbmodel.PrometheusLabelName

	err := metadb.DefaultDB.Unscoped().Find(&prometheusLabelName).Error
	if err != nil {
