...
const (
	SQL_SOURCE_USING_WHERE_COND_ID        = "SOURCE(%s(%sPORT %d USER '%s' PASSWORD '%s' %sDB %s TABLE %s WHERE 'id != 0' INVALIDATE_QUERY 'SELECT updated_at FROM %s WHERE id = 0'))\n"
	SQL_SOURCE_USING_WHERE_COND_DEVICE_ID = "SOURCE(%s(%sPORT %d USER '%s' PASSWORD '%s' %sDB %s TABLE %s WHERE 'deviceid != 0' INVALIDATE_QUERY 'SELECT updated_at FROM %s WHERE deviceid = 0'))\n"
	SQL_SOURCE_USING_WHERE_COND_PID       = "SOURCE(%s(%sPORT %d USER '%s' PASSWORD '%s' %sDB %s TABLE %s WHERE 'pid != 0' INVALIDATE_QUERY 'SELECT updated_at FROM %s WHERE pid = 0'))\n"
	...
	CREATE_DEVICE_DICTIONARY_SQL = SQL_CREATE_DICT +
		...
		SQL_SOURCE_USING_WHERE_COND_DEVICE_ID +
		SQL_LIFETIME +
	...
	CREATE_AZ_DICTIONARY_SQL = SQL_CREATE_DICT +
		...
		")\n" +
		"PRIMARY KEY id\n" +
		SQL_SOURCE_USING_WHERE_COND_ID +
		SQL_LIFETIME +
		SQL_LAYOUT_FLAT
	...
	CREATE_OS_APP_TAG_DICTIONARY_SQL = SQL_CREATE_DICT +
		...
		")\n" +
		"PRIMARY KEY pid, key\n" +
		SQL_SOURCE_USING_WHERE_COND_PID +
		SQL_LIFETIME +
		SQL_LAYOUT_COMPLEX_KEY_HASHED
