...
type ChPodGroup struct {
	SubscriberComponent[*message.PodGroupFieldsUpdate, message.PodGroupFieldsUpdate, metadbmodel.PodGroup, metadbmodel.ChPodGroup, IDKey]
	resourceTypeToIconID map[IconKey]int
}

func NewChPodGroup(resourceTypeToIconID map[IconKey]int) *ChPodGroup {
	mng := &ChPodGroup{
		newSubscriberComponent[*message.PodGroupFieldsUpdate, message.PodGroupFieldsUpdate, metadbmodel.PodGroup, metadbmodel.ChPodGroup, IDKey](
...
func (c *ChPodGroup) onResourceUpdated(sourceID int, fieldsUpdate *message.PodGroupFieldsUpdate, db *metadb.DB) {
	...
	if len(updateInfo) > 0 {
		var chItem metadbmodel.ChPodGroup
