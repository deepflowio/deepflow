...
func (g *GenesisSyncTypeOperation[T]) Load(nodeIP string) {
	...
	for _, db := range mysql.GetDBs().All() {
		...
		for _, storage := range storages {
			...
			dataStrore2[key] = items
		}
		g.lastSeen[db.ORGID] = lastSeen
		g.dataStore2[db.ORGID] = dataStrore2
	}
...
func (g *GenesisSyncTypeOperation[T]) Save(nodeIP string) {
	...
	for _, db := range mysql.GetDBs().All() {
		...
		// delete db old data
		var dataType T
		err = db.Where("node_ip = ?", nodeIP).Delete(&dataType).Error
		if err != nil {
			log.Errorf("delete node (%s) old data failed: %s", nodeIP, err.Error(), logger.NewORGPrefix(db.ORGID))
			continue
		}
		...
		if len(items) > 0 {
			...
			if err != nil {
				log.Warningf("create node (%s) sync data failed: %s", nodeIP, err.Error(), logger.NewORGPrefix(db.ORGID))
			}
		}
	}
