		...
		for {
			...
			if info.MessageType == common.TYPE_RENEW {
				...
				case tridentcommon.TridentType_TT_PHYSICAL_MACHINE:
					genesisSyncData = vUpdater.UnmarshalWorkloadProtobuf(info, common.DEVICE_TYPE_PHYSICAL_MACHINE)
				case tridentcommon.TridentType_TT_PUBLIC_CLOUD:
					genesisSyncData = vUpdater.UnmarshalWorkloadProtobuf(info, common.DEVICE_TYPE_PUBLIC_CLOUD)
				case tridentcommon.TridentType_TT_HOST_POD, tridentcommon.TridentType_TT_VM_POD, tridentcommon.TridentType_TT_K8S_SIDECAR:
					genesisSyncData = vUpdater.UnmarshalKubernetesProtobuf(info)
				default:
					genesisSyncData = vUpdater.UnmarshalProtobuf(info)
				}
				if info.VtapID != 0 {
					genesisSyncDataByVtap[vtap] = genesisSyncData
				}
				vStorage.Update(info, genesisSyncData)
			}
		}
	}()
}
