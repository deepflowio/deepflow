...
func (t *Tencent) getRegions() ([]model.Region, error) {
	...
	resp, err := t.getResponse("vpc", "2017-03-12", "DescribeRegions", "", "RegionSet", true, map[string]interface{}{}, map[string]interface{}{})
	if err != nil {
	...
	for _, rData := range resp {
		...
		name := rData.Get("Name").MustString()
		...
		if len(t.includeRegions) > 0 {
			...
			if regionIndex == len(t.includeRegions) || t.includeRegions[regionIndex] != name {
				log.Infof("region,az region (%s) not in include_regions", name)
				continue
			}
		}
		...
		if len(t.excludeRegions) > 0 {
			...
			if regionIndex < len(t.excludeRegions) && t.excludeRegions[regionIndex] == name {
				log.Infof("region,az region (%s) in exclude_regions", name)
				continue
			}
		}
		...
		t.regionList = append(t.regionList, tencentRegion{
			...
			name:       rRegion,
			regionName: name,
			finance:    rData.Get("IsFinance").MustBool(),
		})
	}
