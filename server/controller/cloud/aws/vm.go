...
func (a *Aws) getVMs(region awsRegion) ([]model.VM, []model.VMSecurityGroup, error) {
	...
	for _, reserve := range retVMs {
		for _, ins := range reserve.Instances {
			...
			azLcuuid := common.GenerateUUID(a.getStringPointerValue(ins.Placement.AvailabilityZone))
			...
			vms = append(vms, model.VM{
				...
				Name:         vmName,
				Label:        instanceID,
				VPCLcuuid:    common.GenerateUUID(a.getStringPointerValue(ins.VpcId)),
				State:        vmState,
			...
			for priority, sg := range ins.SecurityGroups {
				vmSGs = append(vmSGs, model.VMSecurityGroup{
					Lcuuid:              common.GenerateUUID(vmLcuuid + a.getStringPointerValue(sg.GroupId)),
					SecurityGroupLcuuid: common.GenerateUUID(a.getStringPointerValue(sg.GroupId)),
					VMLcuuid:            vmLcuuid,
					Priority:            priority,
				})
			}
