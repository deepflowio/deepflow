...
func (a *Aws) getVInterfacesAndIPs(region awsRegion) ([]model.VInterface, []model.IP, []model.NATRule, error) {
	...
	for _, vData := range retVinterfaces {
		...
		if vData.Attachment.InstanceId != nil {
			...
			vinterfaceLcuuid := common.GenerateUUID(a.getStringPointerValue(vData.NetworkInterfaceId))
			networkLcuuid := common.GenerateUUID(a.getStringPointerValue(vData.SubnetId))
			vpcLcuuid := common.GenerateUUID(a.getStringPointerValue(vData.VpcId))
			...
			for _, ip := range vData.PrivateIpAddresses {
				...
				if err == nil && netPublicIP.Is4() {
					vLcuuid := common.GenerateUUID(vinterfaceLcuuid)
					...
					vNatRules = append(vNatRules, model.NATRule{
						Lcuuid:           common.GenerateUUID(publicIP + vinterfaceLcuuid + privateIP),
						Type:             "DNAT",
						Protocol:         "ALL",
						FloatingIP:       publicIP,
						FixedIP:          privateIP,
