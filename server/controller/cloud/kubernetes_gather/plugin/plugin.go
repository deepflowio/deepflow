...
func GeneratePodGroup(orgID int, db *gorm.DB, metaData *simplejson.Json) (string, string, error) {
	...
	err := db.Where("type = ?", common.PLUGIN_TYPE_LUA).Order("updated_at DESC").Find(&plugins).Error
	if err != nil {
		return "", "", err
	}
...
func customSCIPodGroup(orgID int, metaData *simplejson.Json) (string, string) {
	...
	if providerType != "serverless" && providerType != "proprietary" {
		log.Infof("abstract type (%s) not support", providerType, logger.NewORGPrefix(orgID))
		return "", ""
	}
	...
	if resourceName == "" {
		log.Info("not found provider-resource-name", logger.NewORGPrefix(orgID))
		return "", ""
	}
