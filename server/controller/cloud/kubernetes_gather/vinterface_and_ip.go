...
func (k *KubernetesGather) getVInterfacesAndIPs() (nodeSubnets, podSubnets []model.Subnet, nodeVInterfaces, podVInterfaces []model.VInterface, nodeIPs, podIPs []model.IP, err error) {
	...
	for _, vItem := range vData {
		...
		vinterfaceLcuuid := common.GenerateUUID(podLcuuid + vItem.Mac)
		...
		for _, ipString := range vIPs {
			...
			ipLcuuid := common.GenerateUUID(vinterfaceLcuuid + ip.String())
			subnetLcuuid := ""
			for _, podNetworkCIDR := range k.podNetworkLcuuidCIDRs.cidrs {
				...
				if netPrefix.Contains(ip) {
					subnetLcuuid = common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + podNetworkCIDR)
					break
				}
			}
			if subnetLcuuid == "" {
			...
			if subnetLcuuid == "" {
				if (ip.Is4() && ipMask == 32) || (ip.Is6() && ipMask == 128) {
					...
					subnetLcuuid = common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + rangePrefix.String())
	...
	for _, cidr := range k.podNetworkLcuuidCIDRs.cidrs {
		...
		// 顺便生成已获取的 pod 子网
		pSubnetLcuuid := common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + cidr)
		podSubnets = append(podSubnets, model.Subnet{
	...
	for _, ipPrefix := range noMaskCIDRs {
		subnetLcuuid := common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + "NO_MASK" + ipPrefix.String())
		podSubnets = append(podSubnets, model.Subnet{
			Lcuuid:        subnetLcuuid,
		...
		for _, noMaskIPString := range aggNoMaskNets {
			...
			ipLcuuid := common.GenerateUUID(vinterfaceLcuuid + noMaskIPPrefix.IP().String())
	...
	for _, vItem := range vData {
		...
		for _, ipString := range IPs {
			...
			case k8sNodeIPs.Contains(ipPrefix.IP().String()):
				...
				vinterfaceLcuuid := common.GenerateUUID(k.UuidGenerate + nMAC)
				...
				modelIP := model.IP{
					Lcuuid:           common.GenerateUUID(vinterfaceLcuuid + ipPrefix.IP().String()),
					VInterfaceLcuuid: vinterfaceLcuuid,
					IP:               ipPrefix.IP().String(),
					RegionLcuuid:     k.RegionUUID,
					SubnetLcuuid:     common.GenerateUUID(k.nodeNetworkLcuuidCIDRs.networkLcuuid + rangePrefix.String()),
				}
			...
			case k.PortNameRegex != "" && portNameRegex.MatchString(nName):
				...
				for _, nCIDR := range k.nodeNetworkLcuuidCIDRs.cidrs {
					...
					if nodeIPPrefix.Contains(hostip) {
						nodeSubnetLcuuid = common.GenerateUUID(k.nodeNetworkLcuuidCIDRs.networkLcuuid)
					}
				}
				...
				if nodeSubnetLcuuid == "" {
					for _, pCIDR := range k.podNetworkLcuuidCIDRs.cidrs {
						...
						if podIPPrefix.Contains(hostip) {
							nodeSubnetLcuuid = common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + pCIDR)
						}
					}
					if nodeSubnetLcuuid != "" {
						networkLcuuid = k.podNetworkLcuuidCIDRs.networkLcuuid
				...
				if nodeSubnetLcuuid == "" {
					...
					nodeSubnetLcuuid = common.GenerateUUID(k.nodeNetworkLcuuidCIDRs.networkLcuuid + cidr)
				...
				if nMAC == common.VIF_DEFAULT_MAC {
					vinterfaceLcuuid = common.GenerateUUID(k.UuidGenerate + nMAC + hostip.String())
				} else {
					vinterfaceLcuuid = common.GenerateUUID(k.UuidGenerate + nMAC)
				}
	...
	for _, nCIDR := range k.nodeNetworkLcuuidCIDRs.cidrs {
		nodeSubnetLcuuid := common.GenerateUUID(k.nodeNetworkLcuuidCIDRs.networkLcuuid + nCIDR)
		nodeSubnets = append(nodeSubnets, model.Subnet{
			Lcuuid:        nodeSubnetLcuuid,
	...
	for nodeIP, nodeLcuuid := range k.nodeIPToLcuuid {
		...
		vinterfaceLcuuid := common.GenerateUUID(k.nodeNetworkLcuuidCIDRs.networkLcuuid + nodeLcuuid + common.VIF_DEFAULT_MAC)
		...
		nodeIPs = append(nodeIPs, model.IP{
			Lcuuid:           common.GenerateUUID(vinterfaceLcuuid + nodeIP),
			VInterfaceLcuuid: vinterfaceLcuuid,
			IP:               nodeIP,
			RegionLcuuid:     k.RegionUUID,
			SubnetLcuuid:     nodeSubnetLcuuid,
	...
	for pIP, pLcuuid := range k.podIPToLcuuid {
		...
		vinterfaceLcuuid := common.GenerateUUID(pLcuuid + common.VIF_DEFAULT_MAC)
		if !vinterfaceLcuuids.Contains(vinterfaceLcuuid) {
			vinterface := model.VInterface{
				Lcuuid:        vinterfaceLcuuid,
				Type:          common.VIF_TYPE_LAN,
		...
		modelIP := model.IP{
			Lcuuid:           common.GenerateUUID(vinterfaceLcuuid + pLcuuid),
			VInterfaceLcuuid: vinterfaceLcuuid,
			IP:               pIP,
			RegionLcuuid:     k.RegionUUID,
	...
	if len(podV4Cidrs) != 0 {
		...
		if len(pV4cidrs) == 1 {
			pV4cidr = pV4cidrs[0]
			pV4SLcuuid = common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + pV4cidr)
			podSubnets = append(podSubnets, model.Subnet{
				Lcuuid:        pV4SLcuuid,
				Name:          k.Name + "_POD_NET",
				CIDR:          pV4cidr,
	...
	if len(podV6Cidrs) != 0 {
		...
		if len(pV6cidrs) == 1 {
			pV6cidr = pV6cidrs[0]
			pV6SLcuuid = common.GenerateUUID(k.podNetworkLcuuidCIDRs.networkLcuuid + pV6cidr)
			podSubnets = append(podSubnets, model.Subnet{
				Lcuuid:        pV6SLcuuid,
				Name:          k.Name + "_POD_NET",
				CIDR:          pV6cidr,
	...
	for lcuuid, ip := range podLcuuidToPodIP {
		var sLcuuid string
		vinterfaceLcuuid := common.GenerateUUID(lcuuid + common.VIF_DEFAULT_MAC)
		switch {
		case ip.Is4():
			sLcuuid = pV4SLcuuid
		case ip.Is6():
		...
		modelIP := model.IP{
			Lcuuid:           common.GenerateUUID(vinterfaceLcuuid + lcuuid),
			VInterfaceLcuuid: vinterfaceLcuuid,
			IP:               ip.String(),
			RegionLcuuid:     k.RegionUUID,
