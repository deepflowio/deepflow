...
type CustomAppConfig struct {
	...
	policyIDToPolicyFields sync.Map
	metaData               *MetaData
}
...
func (c *CustomAppConfig) generateCache() {
	...
	// policy field
	bizDecodePolicyFields := metaDataCache.GetBizDecodePolicyFields()
	policyFields := map[int][]metadbmodel.BizDecodePolicyField{}
	for _, f := range bizDecodePolicyFields {
		policyFields[f.PolicyID] = append(policyFields[f.PolicyID], *f)
	}
	for k, v := range policyFields {
		if !changeVersion {
			value, ok := c.policyIDToPolicyFields.Load(k)
			if !ok {
				changeVersion = true
			} else {
				changeVersion = !slices.Equal(value.([]metadbmodel.BizDecodePolicyField), v)
			}
		}
		c.policyIDToPolicyFields.Store(k, v)
	}
...
func (c *CustomAppConfig) GetCustomAppConfigByte(teamID, agentGroupID int) []byte {
	...
	if ok {
		...
		for _, p := range policies.([]metadbmodel.BizDecodePolicy) {
			...
			policyFields, ok := c.policyIDToPolicyFields.Load(p.ID)
			if ok {
				policyFieldYamls := []map[string]interface{}{}
				for _, f := range policyFields.([]metadbmodel.BizDecodePolicyField) {
					policyFieldYaml := koanf.New(".")
					err := policyFieldYaml.Load(rawbytes.Provider([]byte(f.Yaml)), kyaml.Parser())
					if err != nil {
						errMessage := fmt.Sprintf("load policy field (%s) yaml failed: %s", f.Name, err.Error())
						log.Error(errMessage, logger.NewORGPrefix(c.orgID))
						return []byte("# " + errMessage)
					}
					policyFieldYamls = append(policyFieldYamls, policyFieldYaml.Raw())
				}
				err = policyYaml.Set(common.CONFIG_KEY_CUSTOM_FIELD_POLICIE_FIELDS, policyFieldYamls)
				if err != nil {
					errMessage := fmt.Sprintf("set policy field yamls failed: %s", err.Error())
					log.Error(errMessage, logger.NewORGPrefix(c.orgID))
					return []byte("# " + errMessage)
				}
			}
