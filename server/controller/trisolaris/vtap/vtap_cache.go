...
func NewVTapCache(vtap *metadbmodel.VTap, vTapInfo *VTapInfo) *VTapCache {
	...
	vTapCache.sameCluster = vtap.SameCluster
...
func (c *VTapCache) convertLicenseFunctions() {
	...
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_NET_NPB) {
		c.enabledNetNpb.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_NET_NPMD) {
		c.enabledNetNpmd.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_NET_DPDK) {
		c.enabledNetDpdk.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_TRACE_NET) {
		c.enabledTraceNet.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_TRACE_SYS) {
		c.enabledTraceSys.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_TRACE_APP) {
		c.enabledTraceApp.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_TRACE_IO) {
		c.enabledTraceIo.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_TRACE_BIZ) {
		c.enabledTraceBiz.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_PROFILE_CPU) {
		c.enabledProfileCpu.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_PROFILE_RAM) {
		c.enabledProfileRam.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_PROFILE_INT) {
		c.enabledProfileInt.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_LEGACY_METRIC) {
		c.enabledLegacyMetric.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_LEGACY_LOG) {
		c.enabledLegacyLog.Set()
	}
	if c.sameCluster == common.VTAP_SAME_CLUSTER_TRUE || slices.Contains(licenseFunctionsInt, AGENT_LICENSE_FUNCTION_LEGACY_PROBE) {
...
func (c *VTapCache) GetVTapSameCluster() int {
	return c.sameCluster
}

func (c *VTapCache) UpdateSameCluster(isSameCluster bool) {
	if isSameCluster {
		c.sameCluster = common.VTAP_SAME_CLUSTER_TRUE
	} else {
		c.sameCluster = common.VTAP_SAME_CLUSTER_FALSE
	}
}
