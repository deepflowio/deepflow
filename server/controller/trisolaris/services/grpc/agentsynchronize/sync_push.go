...
func (e *AgentEvent) generateUserConfig(c *vtap.VTapCache, clusterID string, gAgentInfo *vtap.VTapInfo, orgID int) string {
	...
	if clusterID != "" { // if agent report cluster_id, force set tridentType = VTAP_TYPE_POD_VM
		userConfig.Set("global.common.agent_type", VTAP_TYPE_POD_VM)
	}

	cacheTSBIP := c.GetTSDBIP()
	configTSDBIP := gAgentInfo.GetConfigTSDBIP()
	if configTSDBIP != "" {
		userConfig.Set("global.communication.ingester_ip", configTSDBIP)
		userConfig.Set("global.communication.ingester_port", DefaultAnalyzerPort)
	} else if cacheTSBIP != "" {
		userConfig.Set("global.communication.ingester_ip", cacheTSBIP)
		userConfig.Set("global.communication.ingester_port", DefaultAnalyzerPort)
	}

	natIPEnabled := userConfig.GetBool("global.communication.request_via_nat_ip")
	if trisolaris.GetAllAgentConnectToNatIP() || natIPEnabled == true {
		userConfig.Set("global.communication.proxy_controller_ip", trisolaris.GetORGNodeInfo(orgID).GetControllerNatIP(c.GetControllerIP()))
		userConfig.Set("global.communication.proxy_controller_port", DefaultProxyControllerPort)

		userConfig.Set("global.communication.ingester_ip", trisolaris.GetORGNodeInfo(orgID).GetTSDBNatIP(c.GetTSDBIP()))
		userConfig.Set("global.communication.ingester_port", DefaultAnalyzerPort)
	}

	if isPodVTap(c.GetVTapType()) && gAgentInfo.IsTheSameCluster(clusterID) {
		userConfig.Set("global.communication.proxy_controller_ip", trisolaris.GetORGNodeInfo(orgID).GetControllerPodIP(c.GetControllerIP()))
		userConfig.Set("global.communication.proxy_controller_port", trisolaris.GetGrpcPort())

		userConfig.Set("global.communication.ingester_ip", trisolaris.GetORGNodeInfo(orgID).GetTSDBPodIP(c.GetTSDBIP()))
		userConfig.Set("global.communication.ingester_port", trisolaris.GetIngesterPort())

	}
	if userConfig.GetString("global.communication.proxy_controller_ip") == "" {
	...
	if userConfig.GetString("global.communication.ingester_ip") == "" {
		userConfig.Set("global.common.enabled", false)
	...
	if c.GetVTapEnabled() == 0 {
		userConfig.Set("inputs.resources.private_cloud.hypervisor_resource_enabled", false)
		userConfig.Set("global.common.enabled", false)
	}
...
func (e *AgentEvent) generateNoAgentCacheUserConfig(groupID string, orgID int) *viper.Viper {
...
func (e *AgentEvent) formateUserConfigToString(userConfig *viper.Viper) string {
...
func (e *AgentEvent) noAgentResponse(in *api.SyncRequest, orgID int) *api.SyncResponse {
	...
	if in.GetKubernetesClusterId() != "" {
		...
		userConfig.Set("global.communication.max_escape_duration", gAgentInfo.GetDefaultMaxEscapeSecondsStr())
		userConfig.Set("global.limits.max_memory", gAgentInfo.GetDefaultMaxMemory())
		userConfig.Set("global.common.agent_type", VTAP_TYPE_POD_VM)
		userConfig.Set("global.common.enabled", false)
	...
	if agentTypeForUnknowAgent != 0 {
		userConfig.Set("global.communication.max_escape_duration", gAgentInfo.GetDefaultMaxEscapeSecondsStr())
		userConfig.Set("global.limits.max_memory", gAgentInfo.GetDefaultMaxMemory())
		userConfig.Set("global.common.agent_type", int(agentTypeForUnknowAgent))
		userConfig.Set("global.common.enabled", false)
		userConfig.Set("inputs.resources.private_cloud.hypervisor_resource_enabled", true)
