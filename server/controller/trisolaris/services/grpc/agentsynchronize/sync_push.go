...
func (e *AgentEvent) generateUserConfig(c *vtap.VTapCache, clusterID string, gAgentInfo *vtap.VTapInfo, orgID int) string {
	...
	if trisolaris.GetAllAgentConnectToNatIP() || natIPEnabled == true {
		userConfig.Global.Communication.ControllerIP = trisolaris.GetORGNodeInfo(orgID).GetControllerNatIP(c.GetControllerIP())
		userConfig.Global.Communication.ControllerPort = DefaultProxyControllerPort
	...
	if isPodVTap(c.GetVTapType()) && gAgentInfo.IsTheSameCluster(clusterID) {
		userConfig.Global.Communication.ControllerIP = trisolaris.GetORGNodeInfo(orgID).GetControllerPodIP(c.GetControllerIP())
		userConfig.Global.Communication.ControllerPort = trisolaris.GetGrpcPort()
	...
	if userConfig.Global.Communication.ControllerIP == "" {
