...
func (e *AgentEvent) generateUserConfig(c *vtap.VTapCache, gAgentInfo *vtap.VTapInfo, isSameCluster bool, orgID int) *koanf.Koanf {
	...
	if isPodVTap(c.GetVTapType()) && isSameCluster {
...
func (e *AgentEvent) Sync(ctx context.Context, in *api.SyncRequest) (*api.SyncResponse, error) {
	...
	isSameCluster := gAgentInfo.IsTheSameCluster(in.GetKubernetesClusterMd5())
	vtapCache.UpdateSameCluster(isSameCluster)
	userConfig := e.generateUserConfig(vtapCache, gAgentInfo, isSameCluster, orgID)
