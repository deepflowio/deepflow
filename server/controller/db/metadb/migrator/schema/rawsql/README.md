...
版本升级文件采用语义化版本号命名，格式为：`{major}.{minor}.{patch}.{build}.sql`
...
### 版本升级文件执行
`issu/`目录中的文件按版本号顺序执行：
- 文件名格式：`{major}.{minor}.{patch}.{build}.sql`
- 按版本号数值大小排序，从小到大执行
...
2. **转换SQL语法**
   - 数据类型映射（如：INTEGER → SERIAL）
   - 语法差异处理（如：AUTO_INCREMENT → SERIAL）
   - 函数调用适配（如：uuid() → gen_random_uuid()）
