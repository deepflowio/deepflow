...
func TestMySQLIssue_7_1_0_38BackfillsAIEventDataSources(t *testing.T) {
	sqlPath := filepath.Join("rawsql", "mysql", "issu", "7.1.0.38.sql")
	content, err := os.ReadFile(sqlPath)
	if err != nil {
		t.Fatalf("read issue sql failed: %v", err)
	}
	sql := string(content)
	...
	for _, collection := range requiredCollections {
		if !strings.Contains(sql, collection) {
			t.Fatalf("issue 7.1.0.38 missing data_source backfill for %s", collection)
		}
	}
}
