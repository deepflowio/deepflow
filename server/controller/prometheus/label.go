...
type ORGLabelSynchronizers struct {
	...
	orgIDToCounter cmap.ConcurrentMap[int, *statsd.PrometheusLabelIDsCounter]
	orgIDToResp    cmap.ConcurrentMap[int, *trident.PrometheusLabelResponse]
}

func (s *ORGLabelSynchronizers) GetStatsdCounter() *statsd.PrometheusLabelIDsCounter {
	counter := statsd.NewPrometheusLabelIDsCounter()
	for iter := range s.orgIDToCounter.IterBuffered() {
		counter.SendMetricCount += iter.Val.SendMetricCount
		counter.SendLabelCount += iter.Val.SendLabelCount
		// counter.SendTargetCount += iter.Val.SendTargetCount
	}
	return counter
}
...
func NewORGLabelSynchronizer() (*ORGLabelSynchronizers, error) {
	return &ORGLabelSynchronizers{
		orgIDToCounter: cmap.NewWithCustomShardingFunction[int, *statsd.PrometheusLabelIDsCounter](common.ShardingInt),
...
func (s *ORGLabelSynchronizers) assembleFully() (*trident.PrometheusLabelResponse, error) {
	...
	for iter := range cache.GetORGCaches().GetORGIDToCache().IterBuffered() {
		common.AppendErrGroup(eg, s.goAssembleFully, iter.Val)
	}
	...
	for iter := range s.orgIDToResp.IterBuffered() {
...
func (s *ORGLabelSynchronizers) goAssembleFully(args ...interface{}) error {
	...
	s.orgIDToResp.Set(synchronizer.org.GetID(), resp)
	s.orgIDToCounter.Set(synchronizer.org.GetID(), synchronizer.GetStatsdCounter())
	return nil
}
...
func (s *ORGLabelSynchronizers) assemble(req *trident.PrometheusLabelRequest) (*trident.PrometheusLabelResponse, error) {
	...
	for orgID, reqLabels := range orgIDToReqLabels {
		common.AppendErrGroup(eg, s.goAssemble, orgID, reqLabels)
	}
	if err := eg.Wait(); err != nil {
...
func (s *ORGLabelSynchronizers) goAssemble(args ...interface{}) error {
	orgID := args[0].(int)
	reqLabels := args[1].([]*trident.MetricLabelRequest)
...
func (s *LabelSynchronizer) setStatsdCounterData() {
