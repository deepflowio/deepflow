...
func (s *LabelSynchronizer) generateDataToEncode(req *trident.PrometheusLabelRequest) *dataToEncode {
	...
	for _, m := range req.GetRequestLabels() {
		...
		targetKey, targetID, targetExists := s.getTargetInfoFromLabels(m.GetLabels(), int(m.GetPodClusterId()))
		if targetExists {
	...
	for _, t := range req.GetRequestTargets() {
		tk := cache.NewTargetKey(t.GetInstance(), t.GetJob(), int(t.GetPodClusterId()))
		if _, ok := s.cache.Target.GetIDByKey(tk); !ok {
			toEncode.tryAppendTarget(tk)
		}
	}
	return toEncode
}
...
func (s *LabelSynchronizer) getTargetInfoFromLabels(labels []*trident.LabelRequest, podClusterID int) (cache.TargetKey, int, bool) {
	...
	targetKey := cache.NewTargetKey(instanceValue, jobValue, podClusterID)
...
func (s *LabelSynchronizer) assembleMetricLabel(mls []*trident.MetricLabelRequest) ([]*trident.MetricLabelResponse, error) {
	...
	for _, ml := range mls {
		...
		_, _, ok := s.getTargetInfoFromLabels(ml.GetLabels(), int(ml.GetPodClusterId()))
...
func (d *dataToEncode) tryAppendTarget(tk cache.TargetKey) {
