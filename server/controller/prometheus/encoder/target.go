...
func (ln *target) encode(ts []*controller.PrometheusTargetRequest) ([]*controller.PrometheusTarget, error) {
	...
	for i := range ts {
		...
		if id, ok := ln.keyToID.Get(cache.NewTargetKey(ins, job, vpcID, podClusterID)); ok {
...
func (ln *target) load() (ids mapset.Set[int], err error) {
	...
	for _, item := range items {
		...
		ln.keyToID.Set(cache.NewTargetKey(item.Instance, item.Job, item.VPCID, item.PodClusterID), item.ID)
	}
