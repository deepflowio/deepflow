...
func (c *Cleaner) deleteAndRefresh() error {
	...
	if orgIDs, err := mysql.GetORGIDs(); err != nil {
		...
		for _, orgID := range orgIDs {
			...
			en, err := encoder.GetEncoder(orgID)
			if err != nil {
				log.Errorf("failed to get encoder: %v", err)
				return err
			}
			en.Refresh()
		}
...
func (c *deleter) deleteExpiredLabel() error {
	...
	for _, item := range c.dataToCheck.labels {
		if !c.activeData.getLabel(item.Name, item.Value) {
			toDelete = append(toDelete, item)
		}
	}
	return DeleteBatch(prometheuscommon.ResourcePrometheusLabel, c.org.DB, toDelete)
}

func (c *deleter) deleteExpiredLabelName() error {
	toDelete := make([]mysql.PrometheusLabelName, 0)
	for _, item := range c.dataToCheck.labelNames {
		if !c.activeData.getLabelName(item.Name) {
			toDelete = append(toDelete, item)
		}
	}
	return DeleteBatch(prometheuscommon.ResourcePrometheusLabelName, c.org.DB, toDelete)
}

func (c *deleter) deleteExpiredLabelValue() error {
	toDelete := make([]mysql.PrometheusLabelValue, 0)
	for _, item := range c.dataToCheck.labelValues {
		if !c.activeData.getLabelValue(item.Value) {
			toDelete = append(toDelete, item)
		}
	}
	return DeleteBatch(prometheuscommon.ResourcePrometheusLabelValue, c.org.DB, toDelete)
}

func (c *deleter) deleteExpiredMetricAPPLabelLayout() error {
	toDelete := make([]mysql.PrometheusMetricAPPLabelLayout, 0)
	for _, item := range c.dataToCheck.metricAPPLabelLayouts {
		if !c.activeData.getMetricLabelName(item.MetricName, item.APPLabelName) {
			toDelete = append(toDelete, item)
		}
	}
	return DeleteBatch(prometheuscommon.ResourcePrometheusMetricAPPLabelLayout, c.org.DB, toDelete)
}
