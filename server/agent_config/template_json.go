	...
	case yaml.MappingNode:
		...
		for i := 0; i < len(node.Content); i += 2 {
			...
			if strings.HasSuffix(key, FieldCommentSuffix) {
				...
				// 	strValNodes := make([]*yaml.Node, 0)
				// 	for j := 0; j < len(valueNode.Content); j++ {
				// 		bytes, err := yaml.Marshal(valueNode.Content[j])
				// 		if err != nil {
				// 			return nil, err
				// 		}
				// 		strValNode := &yaml.Node{
				...
				// 			Value: string(bytes),
				// 		}
				// 		strValNodes = append(strValNodes, strValNode)
				// 	}
				// 	node.Content[i+1].Content = strValNodes
				// }
				dictKey = ""
			} else {
				value, err := convTypeDictValToString(valueNode)
...
func fillingOrder(node *yaml.Node) (interface{}, error) {
	...
	case yaml.MappingNode:
		// fmt.Printf("parentKey: %s,MappingNode: %#v\n", parents, node)
		om := &OrderedMap{
		...
		for i := 0; i < len(node.Content); i += 2 {
			...
			valueNode := node.Content[i+1]
			// fmt.Printf("keyNode: %#v\n", keyNode)
			// fmt.Printf("valueNode: %#v\n", valueNode)
	...
	case yaml.SequenceNode:
		// fmt.Printf("parentKey: %s, SequenceNode: %#v\n", parents, node)
		var seq []interface{}
		for i, contentNode := range node.Content {
			// fmt.Printf("contentNode: %#v\n", contentNode)
