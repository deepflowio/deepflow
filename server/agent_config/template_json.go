...
func ParseYAMLToJson(yamlData []byte) ([]byte, error) {
	...
	jsonData, err := yamlNodeToMapWithOrder(&node, "")
	if err != nil {
...
func yamlNodeToMapWithOrder(node *yaml.Node, parentKey string) (interface{}, error) {
	switch node.Kind {
	case yaml.DocumentNode:
		return yamlNodeToMapWithOrder(node.Content[0], "")
	case yaml.MappingNode:
		...
		for i := 0; i < len(node.Content); i += 2 {
			...
			key := keyNode.Value
			value, err := yamlNodeToMapWithOrder(valueNode, key)
			if err != nil {
				return nil, err
			}
	...
	case yaml.SequenceNode:
		...
		for i, contentNode := range node.Content {
			value, err := yamlNodeToMapWithOrder(contentNode, parentKey)
			if err != nil {
				return nil, err
			}
			if valueMap, ok := value.(map[string]interface{}); ok {
