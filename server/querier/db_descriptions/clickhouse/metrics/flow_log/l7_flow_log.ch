...
captured_request_byte  , 采集的请求字节数  , Byte , 对于 Packet 信号源，表示 AF_PACKET 采集到的包长，且不包括四层头；对于 eBPF 信号源，表示一次系统调用的字节数，注意在开启 TCP 流重组时表示多次系统调用的字节数之和。
captured_response_byte , 采集的响应字节数  , Byte , 对于 Packet 信号源，表示 AF_PACKET 采集到的包长，且不包括四层头；对于 eBPF 信号源，表示一次系统调用的字节数，注意在开启 TCP 流重组时表示多次系统调用的字节数之和。
