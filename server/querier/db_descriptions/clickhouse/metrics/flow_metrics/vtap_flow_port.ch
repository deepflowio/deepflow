...
l7_error                    , 应用异常                , 个   , 异常包含客户端异常 + 服务端异常，根据具体应用协议的响应码判断异常，不同协议的定义见 `l7_flow_log` 中 xx 字段的说明
l7_client_error             , 应用客户端异常          , 个   , 根据具体应用协议的响应码判断异常，不同协议的定义见 `l7_flow_log` 中 xx 字段的说明 
l7_server_error             , 应用服务端异常          , 个   , 根据具体应用协议的响应码判断异常，不同协议的定义见 `l7_flow_log` 中 xx 字段的说明
