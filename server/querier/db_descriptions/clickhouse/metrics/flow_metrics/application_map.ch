...
request                     , 请求                 , 个   , 请求总数
response                    , 响应                 , 个   , 响应总数
direction_score             , 方向得分             ,      , 算法推理应用协议请求方向的得分值，得分越高时请求方向的准确性越高，得分为 255 表示请求方向推理结果绝对正确。

rrt                         , 平均时延             , 微秒 , 采集周期内所有应用时延的平均值，单次应用时延由响应时间与请求时间的时差算得
rrt_max                     , 最大时延             , 微秒 , 采集周期内所有应用时延的最大值，单次应用时延由响应时间与请求时间的时差算得
