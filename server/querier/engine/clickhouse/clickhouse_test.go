...
var (
	parseSQL = []struct {
		...
		output: []string{"SELECT service_id_0 AS `pod_service_id_0` FROM flow_log.`l7_flow_log` WHERE (not(service_id_0 = 3)) GROUP BY service_id_0 AS `pod_service_id_0` LIMIT 10000"},
	}, {
		...
		output: []string{"SELECT dictGet('flow_tag.pod_ns_map', 'name', (toUInt64(pod_ns_id))) AS `pod_ns`, topK(10)(dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id)))) FROM flow_metrics.`application.1h` WHERE `time` >= 1687315761 AND `time` <= 1687316661 GROUP BY dictGet('flow_tag.pod_ns_map', 'name', (toUInt64(pod_ns_id))) AS `pod_ns` LIMIT 10"},
	}, {
		name:   "topk_2",
		db:     "flow_metrics",
		...
		output: []string{"SELECT dictGet('flow_tag.pod_ns_map', 'name', (toUInt64(pod_ns_id))) AS `pod_ns`, any(dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id)))) FROM flow_metrics.`application.1h` WHERE `time` >= 1694069050 AND `time` <= 1694990640 GROUP BY dictGet('flow_tag.pod_ns_map', 'name', (toUInt64(pod_ns_id))) AS `pod_ns` LIMIT 10"},
	}, {
		name:   "any_2",
		db:     "flow_metrics",
		...
		output: []string{"SELECT if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id`, AVGIf(if(l7_error/l7_response>=0, least(l7_error/l7_response, 1), null), l7_response>0)*100 AS `AAvg(l7_error_ratio)`, AVGIf(if(retrans_syn/syn_count>=0, least(retrans_syn/syn_count, 1), null), syn_count>0)*100 AS `AAvg(retrans_syn_ratio)`, AVGIf(if(retrans_synack/synack_count>=0, least(retrans_synack/synack_count, 1), null), synack_count>0)*100 AS `AAvg(retrans_synack_ratio)`, AVGIf(if(l7_client_error/l7_response>=0, least(l7_client_error/l7_response, 1), null), l7_response>0)*100 AS `AAvg(l7_client_error_ratio)`, AVGIf(if(l7_server_error/l7_response>=0, least(l7_server_error/l7_response, 1), null), l7_response>0)*100 AS `AAvg(l7_server_error_ratio)` FROM flow_log.`l4_flow_log` GROUP BY if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id` LIMIT 1"},
	}, {
		...
		output: []string{"SELECT if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id`, AVGIf(if(if(response_status IN [4, 3],1,0)/if(type IN [1, 2],1,0)>=0, least(if(response_status IN [4, 3],1,0)/if(type IN [1, 2],1,0), 1), null), if(type IN [1, 2],1,0)>0)*100 AS `AAvg(error_ratio)` FROM flow_log.`l7_flow_log` GROUP BY if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id` LIMIT 1"},
	}, {
		...
		output: []string{"WITH if(count(`_minus_1__div__sum_error__sum_response`)=1, min(`_minus_1__div__sum_error__sum_response`), 0) AS `min_fillnullaszero__minus_1__div__sum_error__sum_response` SELECT auto_service_id, AVG(`_minus_1__div__sum_error__sum_response`)*100 AS `Avg(success_ratio)`, minus(MAX(`_minus_1__div__sum_error__sum_response`), `min_fillnullaszero__minus_1__div__sum_error__sum_response`)*100 AS `Spread(success_ratio)` FROM (WITH if(SUM(response)>0, if(divide(SUM(error), SUM(response))>=0, least(divide(SUM(error), SUM(response)), 1), null), null) AS `divide_0diveider_as_null_sum_error_sum_response` SELECT if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id`, minus(1, if(`divide_0diveider_as_null_sum_error_sum_response`>=0, least(`divide_0diveider_as_null_sum_error_sum_response`, 1), null)) AS `_minus_1__div__sum_error__sum_response` FROM flow_metrics.`application` GROUP BY if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id`) GROUP BY `auto_service_id` LIMIT 1"},
		db:     "flow_metrics",
	}, {
		...
		output: []string{"SELECT auto_service_id, AVG(`_div__sum_byte__sum_packet`) AS `AAvg(bpp)`, AVG(`_div__sum_retrans_syn__sum_syn_count`)*100 AS `AAvg(retrans_syn_ratio)` FROM (WITH if(SUM(packet)>0, divide(SUM(byte), SUM(packet)), null) AS `divide_0diveider_as_null_sum_byte_sum_packet`, if(SUM(syn_count)>0, if(divide(SUM(retrans_syn), SUM(syn_count))>=0, least(divide(SUM(retrans_syn), SUM(syn_count)), 1), null), null) AS `divide_0diveider_as_null_sum_retrans_syn_sum_syn_count` SELECT if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id`, `divide_0diveider_as_null_sum_byte_sum_packet` AS `_div__sum_byte__sum_packet`, if(`divide_0diveider_as_null_sum_retrans_syn_sum_syn_count`>=0, least(`divide_0diveider_as_null_sum_retrans_syn_sum_syn_count`, 1), null) AS `_div__sum_retrans_syn__sum_syn_count` FROM flow_metrics.`network_map` GROUP BY if(auto_service_type in (0,255),subnet_id,auto_service_id) AS `auto_service_id`) GROUP BY `auto_service_id` LIMIT 1"},
		db:     "flow_metrics",
	}, {
