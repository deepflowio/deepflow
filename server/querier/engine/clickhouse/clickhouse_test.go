...
var (
	parseSQL = []struct {
		...
		input:  "select Uniq(ip_0, region_0) as uniq_0 from l4_flow_log limit 1",
		output: "SELECT uniq((if(is_ipv4=1, IPv4NumToString(ip4_0), IPv6NumToString(ip6_0)), dictGet(flow_tag.region_map, 'name', (toUInt64(region_id_0))))) AS `uniq_0` FROM flow_log.`l4_flow_log` LIMIT 1",
	}, {
		...
		output: "SELECT attribute_values[indexOf(attribute_names,'cc')] AS `attribute.abc` FROM flow_log.`l7_flow_log` PREWHERE attribute_values[indexOf(attribute_names,'cc')] = 'opensource-loki-0' AND (attribute_values[indexOf(attribute_names,'cc')] != '') GROUP BY `attribute.abc` LIMIT 10000",
	}, {
		input:  "select `tag.cc` as `tag.abc` from cpu where `tag.abc`='opensource-loki-0' group by `tag.abc`",
		output: "SELECT tag_values[indexOf(tag_names,'cc')] AS `tag.abc` FROM ext_metrics.`metrics` PREWHERE (virtual_table_name='cpu') AND tag_values[indexOf(tag_names,'cc')] = 'opensource-loki-0' AND (tag_values[indexOf(tag_names,'cc')] != '') GROUP BY `tag.abc` LIMIT 10000",
		db:     "ext_metrics",
	}, {
		...
		output:     "SELECT arrayStringConcat(topKArray(3)(`_grouparray_dictGet(flow_tag.region_map, name, (toUInt64(region_id)))_dictGet(flow_tag.region_map, 'name', (toUInt64(region_id))) != ''`), ',') AS `TopK_3(区域)` FROM (SELECT groupArrayIf(dictGet(flow_tag.region_map, 'name', (toUInt64(region_id))), dictGet(flow_tag.region_map, 'name', (toUInt64(region_id))) != '') AS `_grouparray_dictGet(flow_tag.region_map, name, (toUInt64(region_id)))_dictGet(flow_tag.region_map, 'name', (toUInt64(region_id))) != ''` FROM flow_metrics.`vtap_app_port.1m` PREWHERE (`time` >= 1705370520 AND `time` <= 1705371300)) LIMIT 10000",
		db:         "flow_metrics",
		datasource: "1m",
	}, {
		index:  "Any_1",
		...
		output: "SELECT dictGet(flow_tag.pod_map, 'name', (toUInt64(pod_id))) AS `pod` FROM flow_metrics.`vtap_app_port` WHERE (auto_instance_type not in (101,102)) AND (auto_service_type not in (10)) LIMIT 1",
		db:     "flow_metrics",
	}, {
