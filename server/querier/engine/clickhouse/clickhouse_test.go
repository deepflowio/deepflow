...
var (
	parseSQL = []struct {
		...
		output:     []string{"WITH toStartOfInterval(time, toIntervalSecond(60)) + toIntervalSecond(arrayJoin([0]) * 60) AS `_toi` SELECT dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod`, toUnixTimestamp(`_toi`) AS `toi`, divide(sum(byte)/(60/60), 60) AS `流量速率` FROM flow_metrics.`network.1m` PREWHERE (pod) GLOBAL GLOBAL IN (SELECT dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod` FROM flow_metrics.`network.1m` PREWHERE `time` >= 1705040184 AND `time` <= 1705045184 AND (pod_id!=0) GROUP BY dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod` LIMIT 5) AND `time` >= 1705040184 AND `time` <= 1705045184 AND (pod_id!=0) GROUP BY `toi`, dictGet('flow_tag.pod_map', 'name', (toUInt64(pod_id))) AS `pod` ORDER BY `toi` desc LIMIT 10000"},
	}, {
