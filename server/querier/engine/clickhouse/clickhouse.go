...
func createTopKColumn(functionAs, prefix string, elementIndex int) (string, string, error) {
	if functionAs == "" {
	...
	columnValue := fmt.Sprintf("arrayStringConcat(tupleElement(`%s`,%d),',')",
		strings.Trim(functionAs, "`"), elementIndex)
	...
	case *sqlparser.FuncExpr:
		...
		if name == view.FUNCTION_TOPK {
			...
			if argLength == 2 {
				...
				topKStr, topKStrAs, err := createTopKColumn(functionAs, "", TUPLE_ELEMENT_VALUES_INDEX)
				if err != nil {
					return err
				}
				topKCounts, topKCountsAs, err := createTopKColumn(functionAs, "counts_", TUPLE_ELEMENT_COUNTS_INDEX)
				if err != nil {
					return err
				}
