...
func (t *WhereTag) Trans(expr sqlparser.Expr, w *Where, e *CHEngine) (view.Node, error) {
	...
	if db == "flow_tag" {
		filter := ""
		switch t.Tag {
		case "value", "devicetype", "device_type", "tag_name", "field_name", "field_type":
		...
		default:
			...
			case "ip_resource_map", "ip_relation_map":
			...
			case "pod_ns_map", "pod_group_map", "pod_service_map", "pod_map", "chost_map":
				if strings.HasSuffix(t.Tag, "_id") {
					...
					if t.Tag == strings.Trim(table, "_map") {
						tagItem, ok := tag.GetTag("display_name", db, table, "default")
						if ok {
							switch strings.ToLower(op) {
							case "match":
						...
						if ok {
							switch strings.ToLower(op) {
							case "match":
								filter = fmt.Sprintf(tagItem.WhereRegexpTranslator, t.Tag, t.Tag, "match", t.Value)
							case "not match":
								filter = "not(" + fmt.Sprintf(tagItem.WhereRegexpTranslator, t.Tag, t.Tag, "match", t.Value) + ")"
							case "not ilike":
								filter = "not(" + fmt.Sprintf(tagItem.WhereTranslator, t.Tag, t.Tag, "ilike", t.Value) + ")"
							case "not in":
								filter = "not(" + fmt.Sprintf(tagItem.WhereTranslator, t.Tag, t.Tag, "in", t.Value) + ")"
							case "!=":
								filter = "not(" + fmt.Sprintf(tagItem.WhereTranslator, t.Tag, t.Tag, "=", t.Value) + ")"
							default:
								filter = fmt.Sprintf(tagItem.WhereTranslator, t.Tag, t.Tag, op, t.Value)
							}
