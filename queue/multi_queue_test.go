...
func TestSingleQueueSingleUserPuts(t *testing.T) {
	...
	keys := []rawqueue.HashKey{0, 7, 7, 6, 7, 6}
	...
	go func() {
		...
		if count := queue.Gets(6, outBatch); count != 2 || outBatch[0] != 10082 || outBatch[1] != 10083 {
			t.Errorf("Expected 2 values: 10082 & 10083, actually %d values: %d & %d", count, outBatch[0], outBatch[1])
		}
		if count := queue.Gets(7, outBatch); count != 2 || outBatch[0] != 10084 || outBatch[1] != 10085 {
...
func TestMultipleQueueMultipleUserPuts(t *testing.T) {
	...
	wg.Add(userCount)
	userPuts := func(i int) {
		keys := []rawqueue.HashKey{rawqueue.HashKey(i), 5, 7, 7, 6, 7, 6}
	...
	wg.Wait()
	if count := queue.Gets(5, outBatch); count != userCount {
		t.Errorf("Expected %d values, actually %d values", userCount, count)
	}
	if count := queue.Gets(6, outBatch); count != userCount*2 {
		t.Errorf("Expected %d values, actually %d values", userCount*2, count)
	}
	if count := queue.Gets(7, outBatch); count != userCount*3 {
