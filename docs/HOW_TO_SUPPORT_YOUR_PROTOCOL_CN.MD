...
定义 L7ProtocolSendLog 结构，主要用于将不同的应用层协议转换成统一的结构发送到 deepflow-server：
```rust
...
- 由于网络环境和协议的复杂性，有可能会接收到不完整的应用层数据帧
  - 例如 MTU 限制导致 IP 分片，TCP 对端接受窗口或者流控拥堵窗口收缩，MSS 过小等原因导致获取不到完整的应用层数据帧
  - 目前尚未实现传输层连接跟踪
