...
var (
	parseSQL = []struct {
		...
		output: "WITH if(count(_sum_byte_tx)=121, min(_sum_byte_tx), 0) AS min_fillnullaszero__sum_byte_tx SELECT AVG(_sum_byte_tx) AS avg_byte_tx FROM (SELECT SUM(byte_tx) AS _sum_byte_tx FROM flow_log.l4_flow_log PREWHERE `time` >= 60 AND `time` <= 180) HAVING minus(MAX(_sum_byte_tx), min_fillnullaszero__sum_byte_tx) >= 0",
	}, {
		...
		output: "WITH if(count(_sum_byte_tx)=121, min(_sum_byte_tx), 0) AS min_fillnullaszero__sum_byte_tx SELECT minus(MAX(_sum_byte_tx), min_fillnullaszero__sum_byte_tx) AS spread_byte_tx FROM (SELECT SUM(byte_tx) AS _sum_byte_tx FROM flow_log.l4_flow_log PREWHERE `time` >= 60 AND `time` <= 180)",
	}, {
		input:  "select Rspread(byte_tx) as rspread_byte_tx from l4_flow_log where `time`>=60 and `time`<=180",
		output: "WITH if(count(_sum_byte_tx)=121, min(_sum_byte_tx), 0) AS min_fillnullaszero__sum_byte_tx SELECT divide(MAX(_sum_byte_tx)+1e-15, min_fillnullaszero__sum_byte_tx+1e-15) AS rspread_byte_tx FROM (SELECT SUM(byte_tx) AS _sum_byte_tx FROM flow_log.l4_flow_log PREWHERE `time` >= 60 AND `time` <= 180)",
	}, {
		...
		output: "SELECT dictGet(deepflow.device_map, 'name', (toUInt64(11),toUInt64(l3_device_id_0))) AS pod_service_0 FROM flow_log.l7_flow_log PREWHERE (not(((if(is_ipv4=1,IPv4NumToString(ip4_0),IPv6NumToString(ip6_0)),toUInt64(l3_epc_id_0)) IN (SELECT ip,l3_epc_id from deepflow.ip_relation_map WHERE pod_service_name = 'xx')) OR (toUInt64(service_id_0) IN (SELECT pod_service_id from deepflow.ip_relation_map WHERE pod_service_name = 'xx')))) AND (l3_device_id_0!=0 AND l3_device_type_0=11) GROUP BY dictGet(deepflow.device_map, 'name', (toUInt64(11),toUInt64(l3_device_id_0))) AS pod_service_0",
	}, {
		input:  "select pod_ingress_0 from l7_flow_log where pod_ingress_0 !='xx' group by pod_ingress_0",
		output: "SELECT pod_ingress_0 FROM flow_log.l7_flow_log PREWHERE (not(((if(is_ipv4=1,IPv4NumToString(ip4_0),IPv6NumToString(ip6_0)),toUInt64(l3_epc_id_0)) IN (SELECT ip,l3_epc_id from deepflow.ip_relation_map WHERE pod_ingress_name = 'xx')) OR (toUInt64(service_id_0) IN (SELECT pod_service_id from deepflow.ip_relation_map WHERE pod_ingress_name = 'xx')))) GROUP BY pod_ingress_0",
	}, {
